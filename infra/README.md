...
| GitHub deploy roles | Four, all in `infra/github_oidc.tf`, one per lane. Trust on every one pins exactly **`aud`, `sub`, `job_workflow_ref`** with `StringEquals` — **not `repository_owner`**: that condition was tried and made every assume-role attempt fail (`Not authorized to perform sts:AssumeRoleWithWebIdentity`), removed in ws #941 after a bisect. It is redundant regardless, since `sub` and `job_workflow_ref` both already begin with the owner. `job_workflow_ref` is the load-bearing one: `sub` pins the *ref*, not the *workflow*, and a `pull_request_target` run reports `GITHUB_REF` as the base branch. |
| ├ `myblog-github-front-deploy` | `myblog_front` `main` / `main.yml`. `s3:ListBucket` on `myblog-prod-web`, `s3:PutObject`+`s3:DeleteObject` under it, `cloudfront:CreateInvalidation` on `E2Q2JH5EAYVU1O`. No `s3:GetObject` (sync compares against `ListObjectsV2`), no `s3:PutObjectAcl` (bucket is `BucketOwnerEnforced`, ACLs disabled). |
| ├ `myblog-github-backend-deploy` | `myblog_backend` `main` / `deploy.yml`. `lambda:UpdateFunctionCode` on `ratemymusic-api` only. |
| ├ `myblog-github-music-deploy` | `myblog_music` `main` / `deploy.yml`. `lambda:UpdateFunctionCode` on `musicApi` only. |
| └ `myblog-github-worker-deploy` | `myblog_worker` `main` / `deploy.yml`. `lambda:UpdateFunctionCode` + `lambda:GetFunction` + `lambda:InvokeFunction` on `blogWorkerLambda` only. `GetFunction` is for `aws lambda wait function-updated-v2` — **the v1 waiter polls `GetFunctionConfiguration`, which no role here grants**, so the workflow must keep the v2 waiter. `InvokeFunction` is the post-deploy health check. |
| Deploy credentials | **No static AWS key exists in any repo** (2026-08-28). All six repos have zero `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` secrets, and IAM user `github-actions-deploy` has zero access keys — its `lambda-deploy-only` policy is retained but unused, and still lists a dead `publisher-github` ARN. **The AWS root account still has two active access keys** (`access_key_1`, unused since 2025-12-10; `access_key_2`, last used 2026-08-26 for the final static-key front deploy). They can only be removed by a root sign-in with MFA — owner action, `SEC-system-hardening` Step 3d-5. |
...
| Secrets (SSM) | `/myblog/backend`, `/myblog/music`, `/myblog/worker` (one per service; SSM SecureString, leading slash; `SECRETS_PARAM` env var; fetched once per cold start via `@lru_cache`). Plus `/myblog/spotify` — Spotify user OAuth client_id/secret + refresh_token (`SPOTIFY_SECRETS_PARAM`; read by worker for `/me/player/*`, by backend for connection status), `/myblog/smoke` — the smoke user's Cognito credentials, and `/myblog/nightly-agent` — the nightly draft agent's Cognito credentials (see "Nightly draft agent" below). Secrets Manager emptied per CHORE-secrets-ssm-migration. **All three boot-time loaders fail closed on a required key** — a value that parses as JSON but omits one raises `ValueError` at Lambda import (naming the parameter and the missing keys, never a value) instead of booting into a broken state. Required: `/myblog/backend` -> `DATABASE_URL`, `EDGE_SECRET`; `/myblog/music` and `/myblog/worker` -> `DATABASE_URL`, `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`. Optional (absence degrades one feature, never the service): backend `GITHUB_TOKEN` (owner-only publishing; logs a WARNING), worker `LASTFM_API_KEY` and `GENIUS_ACCESS_TOKEN` (their jobs no-op). The three checks are **not identical**, and the difference is deliberate: backend inspects the SSM *payload* and also rejects whitespace-only, non-string and non-object values, because `Settings.DATABASE_URL` carries a localhost default that makes the `if not settings.X` form music and worker use a silent no-op there. Consequence when hand-editing: **dropping a required key takes that function down at its next cold start**, not at request time — deliberate, since the backend previously booted on the localhost DB default and 403'd every CloudFront request. `/myblog/spotify` is read per request rather than at boot and degrades to a 503. |
...
- **WAFv2 Web ACL — attached but NOT a control** (OPS-safety-net-drift Step 2, 2026-07-28) — the CloudFront-scope ACL `CreatedByCloudFront-72420c9a` has `RuleCount: 0` + default `Allow` (audit SEC-1): it inspects nothing and blocks nothing. The owner chose to delete it, and **AWS refused**: `InvalidArgument: Distributions with a pricing plan subscription must have a web ACL resource` — this distribution is on the CloudFront **Free flat-rate pricing plan** (same plan that blocks custom cache/response-headers policies, see gotchas above), which force-attaches an ACL. So the empty shell is a **pricing-plan artifact, not a forgotten security control**; the console auto-created it at plan subscription. Treat the security posture as "no WAF": abuse exposure is bounded by Lambda reserved concurrency + the Cognito authorizer on every mutation route. The ACL is console-created and rule-invisible to `plan`, so the honesty check is manual: `aws wafv2 get-web-acl --scope CLOUDFRONT --region us-east-1 --name CreatedByCloudFront-72420c9a --id 02494b05-2403-41eb-8e1f-21b161bee794` → if `RuleCount` is 0 this line stays; if someone adds real rules, update this line to call it a control. (WAFv2 cannot attach to the HTTP API in any case — no API Gateway v2 support; see `reference-waf-http-api-invoke-domain`.)
