...
| "What's currently playing" (track/queue/playing/device) | **should be** `lib/playback/session.ts` | **Updated 2026-08-06** (stale since Step 3a/3b shipped): `NowPlaying.tsx`'s `useNowPlaying()` now subscribes to `playbackSession` via `useSyncExternalStore` for track-identity+anchor (Step 3b, front #361) — tier/mode/like/device/reconnect stay independently local by design. `LyricsViewer.tsx` still calls `readLivePlayback()` fully independently (Step 3c, not started, correctly gated). Neither `readLivePlayback()` itself nor any tracker's call to it is single-flight/cached — a single `MYBLOG_PLAYBACK_CHANGED` still fans out to 3 uncoordinated HTTP reads (`session.ts`/`NowPlaying`/`LyricsViewer`). A newly-found, narrower gap: `NowPlaying`'s own mount effect fires 2 unguarded concurrent reads of its own (`sync()` + `playbackSession.syncFromLive()`), tracked as `BUG-22`/`BUG-28` (`plan.md`) | **Partially consolidated.** Step 3a (front #360) + Step 3b (front #361) shipped 2026-08-05; Step 3c (`LyricsViewer`) still not started, correctly gated on 3b being prod-stable. `BUG-28` is a follow-up inside 3b's own shipped code, not a reopening. Full re-verification → session scratchpad `inv-4-playback-state-ownership.md` |
