...
| `lyrics_demand_source` | manual operator nudge; scheduled invocation uses EventBridge directly | — | run the bounded V57 album-demand source collector. Optional positive integer `limit` (track evaluations, default `LYRICS_DEMAND_BATCH_LIMIT=150`) and `job_limit` (catalog jobs, default `LYRICS_DEMAND_JOB_LIMIT=50`). Serves existing live demand only; never creates demand or translates inline. Duplicate invocations preserve versioned source/work state. |
