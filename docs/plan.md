...
- **ARCH-album-context-panel** (**accepted; Step 1 in-progress this session, single-PR**) — replace the memo window's inline research-note expansion with a shared right-side context panel (가사 / 리서치 노트 tabs, per-tab scroll/state preserved), generalized from the existing dockable lyrics sheet instead of a separate window. Also fixes `useDockTear`'s overly sensitive auto-dock (a floating panel nudged near the slot without ever leaving it could re-dock) with a leave-then-re-enter arm gate; mobile keeps a single floating tabbed panel, no docked layout. Frontend-only. → `docs/rfcs/ARCH-album-context-panel.md`.
