...
- **FEAT-post-edit-delete-ui** — author-facing edit/delete for **published** posts. Backend (`PUT`/`DELETE`/`PATCH restore`) + editor `?id=` republish already work; gaps are the missing UI entry points (`/drafts` 발행됨 tab + `/blog/[slug]` author 편집 link) and un-publish not removing the static MDX. Steps 1–2 frontend-only; Step 3 (MDX removal on delete) gated on an open question (backend vs frontend). RFC: `docs/rfcs/FEAT-post-edit-delete-ui.md` (**accepted** — Step 1/2 ready; Step 3 still gated on Open Q1). Unblocks FEAT-genre-taxonomy item (5).
