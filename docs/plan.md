...
- **D-2** — `LLMTransientError` is never retried (`cli_engine.py:131-146` retries only `LLMValidationError`); one blip costs a whole night, with no alert.
- **E-2** — `/collection/` bylines a raw Cognito-sub fragment (`@user-0468fd3c`) as a display name on a public page.
- **E-4** — 8 homepage `button.nrl-open` controls fail WCAG 2.5.3 "Label in Name" (visible text not contained in `aria-label`).
- **E-6** — `.lf-artist-link` is emitted site-wide by the `AlbumOverlay` island but styled only in `member/layout.css` (loads on `/collection/` + `/settings/` only), so there is no hover affordance anywhere else; **4th recurrence** of the documented `member.css` trap, live-verified.
- **DEP-2** — `@astrojs/rss` 4.0.13 → 4.0.19; **hygiene only**. Applicability was overturned in re-review: the advisory's `source.title`/`enclosure.type` are never populated and the feed emits zero items.
