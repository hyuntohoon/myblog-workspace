...
- **FEAT-member-dashboard-realdata** (in-progress) — `/profile` durable data: (1) **draft** reviews in 평론 tab (reuse `GET /api/posts?status=draft` + `album_cover_url` on `PostListItem`); (2) accumulated **들은 앨범** = aggregate of existing `spotify_play_events` (**no new table**, D-A); (3) **최근 재생 트랙** = new `spotify_recent_tracks` rolling cache (**V14**, D-B; track data already in the recently-played payload, no new Spotify scope); (4) **재생 중 / 최근 재생** via `now_playing` + recent-tracks head (D-C). Goal 5 = separate the 5 listening/writing concepts. Foundation for `FEAT-ai-editorial-critique`; shares `spotify_play_events` with `FEAT-genre-artist-distribution` (distinct surface — no overlap). RFC → `docs/rfcs/FEAT-member-dashboard-realdata.md`. Cross-repo (shared_db V14 → worker → backend+contract → front-in-worktree). ⚠️ `myblog_front` has a concurrent `chore/copy-cleanup` session editing the same member components → front step in isolated worktree off `origin/main`. Status **in-progress** — RFC owner-accepted 2026-06-08; implementation underway (shared_db V14 → worker → backend+contract → front).
