...
- **FEAT-album-review-authoring** (**in-progress; Steps 1+2 SHIPPED + prod-verified**) — album **평가 = rating**(public star rating + one-line comment), **평론 = review**(editor long-form review). Korean UI must not use "리뷰" for either concept. Step 1 shipped ≤60-char one-line rating comments + private `review_candidate`, extending the existing `album_reviews` state without a new table. Step 2 shipped rating count/average/distribution, sort by newest/rating/name, rating history, and editorial-candidate list. Entrance-link/visibility defects found after Step 2 were fixed and prod-verified.
  ...
  **Open owner decision 2026-08-15 — does this baseline reset again?** front #406 added a 담기 entrance to the `/search` album grid, which until then had open-only tiles. `/search` is the most direct "find an album" path in the product, so the funnel this gate measures is not the same funnel it was on 2026-08-09. That is exactly the reasoning that invalidated the `aebabe7` baseline — a counter measured against a known-incomplete entrance. **Claude did not reset it**: whether the new entrance is material enough to restart the count is the owner's call, and it depends on how far the counter has already run (near 10 → finish the current measurement first; still low → reset now and lose nothing). Nothing else about the gate changes either way.
