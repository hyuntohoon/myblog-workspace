...
- **FEAT-home-redesign — FRONT shipped + prod-live 2026-06-14** (front-only; no contract/infra). myblog_front PR **#151** (`746591c`), prod smoke PASSED on `/` + `/canon`. Built A–G: editorial home shell (writer strip + personalizable module stack, 홈 편집 toggle) + BNM hero (30-day rolling) + Latest + Browse-by-genre teaser (links into full `/genres`, kept separate) + By-the-numbers honest counts + footer rewrite + new `/canon` (≥4.0★). Stars-only honored. RFC → `docs/rfcs/FEAT-home-redesign.md` (Status still `draft`; done-promotion pending owner approval, rule 5). **Step 6 (`/blog → /review`) extracted 2026-06-14 → its own RFC `FEAT-blog-to-review-migration` (Backlog below, implemented + PR open). With Step 6 extracted, home-redesign's own scope (steps 1–5 + `/canon`) is complete — only owner promotion (rule 5) remains.**
...
- **FEAT-blog-to-review-migration** (draft — **implemented, front PR open, NOT merged**) — `/blog/* → /review/[slug]` URL migration extracted from `FEAT-home-redesign` Step 6. Front-only: read route `git mv` to `/review/[slug]`, static no-404 redirect stubs for every old `/blog/*` URL (200 + client redirect; true 301 via CloudFront Function = optional owner-applied Step 2), `/blog/category` folded into `/reviews`, internal-link + rss + sitemap sweep. RFC → `docs/rfcs/FEAT-blog-to-review-migration.md`. Verified: lint/astro-check/build all green; awaiting owner manual redirect click-through + merge. Branch `feat/blog-to-review-migration` (ws + front).
