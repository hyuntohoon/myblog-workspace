...
- **FEAT-album-research-notes** (**accepted 2026-06-11**, in progress) — per-bucket opt-in auto AI research notes (scope modes, dedicated researchSQS/Lambda, opus 4.8 + web_search, `album_research` table, restart/refine, writer-GUI display in BucketBoard + `/write` gutter). RFC → `docs/rfcs/FEAT-album-research-notes.md`. **Steps 1 (shared_db) + 2 (infra) running in parallel** (RFC marks them additive); 3 worker → 4 backend → 5 front sequential after. Owner-manual gates: `terraform apply`, `myblog/anthropic` console key, prod DDL apply.
