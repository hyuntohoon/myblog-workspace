...
- **ARCH-bucket-album-modal-unification** (**accepted 2026-08-09; Step 1 not yet started**) — unify the bucket album modal so clicking any bucket tile always opens the same modal shape, surfacing 평가(rating)/리서치 노트/메모 together instead of today's state-dependent branch (`MemoWindow` vs. `StandardModal`) where 평가 has no bucket-side entry at all and 리서치 노트's tile-corner entry is confirmed overlapped and effectively unclickable by a later-added `.bb-tile-mark` (✎ mark, 2026-07-31) sharing the identical `left:6px/bottom:6px` corner as `.rsh-cover-badge` (2026-06-11) — an unreviewed regression, not a design decision. Closes the specific unaudited gap in `FEAT-album-review-authoring`'s own diagnosis (bucket-click path never checked by that RFC's 2026-08-02 entrance audit). Sibling of `ARCH-entity-interaction-domain-audit` Step 5 (memo naming, still that RFC's own step). Owner explicitly rejected both a 3-tab-merge option and an independent Opus alternative (rating control inside the existing separate `MemoWindow`) in favor of full unification. Owner also resolved Open question 4: `FEAT-album-review-authoring`'s 2026-08-12 re-measurement gate restarts its clock at this RFC's Step 1 ship date instead. → `docs/rfcs/ARCH-bucket-album-modal-unification.md`.
