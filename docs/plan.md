...
- **FEAT-playback-bucket-player** (**accepted; Steps 2–8 SHIPPED; owner status promotion pending**) — per-user system-owned playback bucket; album→ordered tracks expansion; queue projection; docked/detachable playback panel; queue-replacement playback; cross-tab single-owner playback; live-playback adoption; and playback entry fixes are complete. The queue remains our own ordered list rather than Spotify's queue; playback reuses `FEAT-member-player`'s existing `play(intent)` ladder. No polling(D28).
  ...
  **Step 8 SHIPPED 2026-08-09** (front #391). Current-state audit found the drag half was already done as a side effect of `ARCH-entity-interaction-v2` Step 5 and `ARCH-album-card-contract-and-composition` Stage 9; the one real gap — `AddToBucketMenu`'s mobile/non-drag tap picker 400ing on any album dropped onto the Playback Bucket, since it never routed through `expand_album_tracks` — is fixed and live-verified (desktop + mobile 390, plus a real trusted-CDP drag that closes a live-verification gap the album-card RFC's own session left open). All nine steps of this RFC are now shipped; remaining item is owner status promotion only. → `docs/rfcs/FEAT-playback-bucket-player.md`.
...
- **DATA-multidisc-track-order** (**draft RFC written 2026-08-09**) — the `tracks` table has **no `disc_no`/`disc_number` column at all** (ORM `myblog_shared_db/src/myblog_shared_db/models.py:273-321`, canonical schema `tests/canonical_schema.sql:265-279`, zero hits across all 50 migrations — it was never added, not dropped). `myblog_music`(`track_repo.py:163`) and `myblog_worker`(`sync_service.py:174`) both read only Spotify's `track_number` and never reference `disc_number`, so a schema fix needs a twin-drift sweep across both — `disc_number` is discarded before it ever reaches a typed field, including in the `CandidateTrackItem` SQS contract shape. **Live-data spot check done 2026-08-09** (the referenced `inv-8-multidisc-ordering.md` never existed in the workspace — ran fresh against prod): `GROUP BY album_id, track_no HAVING count(*) > 1` finds **77 of 3313 albums (~2.3%)** with a repeated `track_no`, both classical multi-work compilations (e.g. "Beethoven: Complete Piano Sonatas", 50 tracks / 14 distinct `track_no`) and clean 2-disc mainstream releases (Nirvana "In Utero (Deluxe Edition)", Hamilton OBC, "SOS Deluxe: LANA", "SWAG II", Rolling Stones "Forty Licks"). Pulling `In Utero (Deluxe Edition)` under `ORDER BY track_no ASC NULLS LAST, created_at` confirms real interleaving, not just duplicate `track_no` values sitting adjacent: disc 1 "Serve The Servants" and disc 2 "Serve The Servants - 2013 Mix" both land at `track_no=1` with identical `created_at` (same sync batch), so the tie-break falls through to arbitrary `id` (UUID) order — the 2013 Mix track sorts first.

  Managed as a data-model migration, independent of the card/playback-state refactors: (1) `myblog_shared_db` migration adding `tracks.disc_no INTEGER` + prod apply **before merge**; (2) backfill the ~77 currently-colliding albums via a one-off Spotify re-fetch (not locally computable) plus the `myblog_music`/`myblog_worker` twin fix to start collecting `disc_number` going forward; (3) switch every `ORDER BY` to `(disc_no, track_no)` — the RFC's own current-state audit found **4 sites**, not the 1 originally cited here (`bucket_service.py` plus 3 more in `myblog_music/track_repo.py`); (4) OpenAPI re-export → contract merge → frontend type regeneration (no frontend logic change). Draft not yet owner-approved. → `docs/rfcs/DATA-multidisc-track-order.md`.

- **ARCH-overlay-modal-isolation** (**implementation complete; owner status promotion pending**) — Steps 1–3 (scroll-isolation half) SHIPPED 2026-08-09 (front — this branch), closing the systemic overlay gap: all 13 re-audited gap components plus `AddToBucketMenu` now lock background scroll (`useDismissable` gained a `lockScroll` option; the 3 components without `useDismissable` call `useScrollLock()` directly), and every locked modal's actual scrolling container gained `overscroll-behavior: contain`. `component-map.md`'s overlay registry now carries a scroll-lock column. `pnpm test`/`lint`/`astro check` all clean; live-browser CDP verification (desktop + 390×844) covered the one publicly-reachable overlay (genre map's `Peek`) — the remaining 12 are member-auth-gated and were verified via jsdom tests only, to avoid exposing the smoke-test password as a tool-call argument. Open questions 2 (`inert`/`aria-hidden`) and 3 (real iOS device touch-scroll verification) remain open, non-blocking per the RFC's own Non-goals. → `docs/rfcs/ARCH-overlay-modal-isolation.md`.

- **ARCH-bucket-album-modal-unification** (**accepted 2026-08-09; Step 1 SHIPPED 2026-08-09**, front #392, `bd56910`) — any writable bucket album with a bucket-item handle now always opens the same unified modal (`MemoWindow`), regardless of publish state, surfacing 평가(live star + one-liner, replacing the dead "미평가" placeholder)/메모/리서치 노트(collapsible) together. The tile's numeric score badge now shows whenever a rating exists, independent of `bucket.isDone`. `CoverResearchBadge`'s corner dot now opens the same unified modal (focused to research) instead of a separate `rsh-modal`; `.bb-tile-mark`/`.rsh-cover-badge` corner stacking is now explicit (`z-index` 1/2, dot grown 12px→16px) rather than implicit — live CDP hit-testing found the collision did not actually reproduce given the dot's pre-existing `z-index:2`, but the fix was applied anyway since accidental-correct stacking is exactly how that regression happened the first time. Verified live (desktop + 390×844 CDP) against a local backend pointed at prod Neon: real PUT round-trip persists on reload, cross-island tile-badge sync via `ENT_ALBUM_STATE_CHANGED`, test data cleaned up afterward. Prod smoke 19/19 pass post-deploy. Owner explicitly rejected both a 3-tab-merge option and an independent Opus alternative (rating control inside the existing separate `MemoWindow`) in favor of full unification. `FEAT-album-review-authoring`'s re-measurement gate baseline reset to this ship date (see that row). Remaining: owner status promotion only. → `docs/rfcs/ARCH-bucket-album-modal-unification.md`.
