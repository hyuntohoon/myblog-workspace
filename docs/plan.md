...
- **FEAT-album-research-notes** (**accepted 2026-06-11**, in progress) — per-bucket opt-in auto AI research notes (scope modes, dedicated researchSQS/Lambda, opus 4.8 + web_search, `album_research` table, restart/refine, writer-GUI display in BucketBoard + `/write` gutter). RFC → `docs/rfcs/FEAT-album-research-notes.md`.
  ...
  - **Remaining gate before Step 3 — owner-only**: create an Anthropic API key at console.anthropic.com and set it as `ANTHROPIC_API_KEY` in Secrets Manager `myblog/anthropic` (container exists, no value yet). Claude cannot mint a console key (no account access); once provided, Claude can write it to the secret. Nothing else blocks Step 3.
