...
- **FEAT-spotify-library-sync** (draft — owner-requested 2026-06-08; RFC Status promotion still owner-only) — D11 follow-up: a NEW special bucket (`review_buckets.kind='spotify_library'`) in the BucketBoard that **explicitly** two-way mirrors the owner's Spotify saved-albums Library (`GET/PUT/DELETE /me/albums`, album-level, no playlists); per-album source (myblog_added/preexisting) + state, never deletes a pre-existing saved album. Implements original spec reqs **4/5/6** (1/2/3 already shipped #287). RFC → `docs/rfcs/FEAT-spotify-library-sync.md`. **Order**: Step 0 owner token re-bootstrap (`user-library-read`+`user-library-modify`) → shared_db V15 → worker → backend (contract) → front. **Migration: V15** (genre re-bumps to V16). All Spotify I/O worker-async (rule #9); first prod reconcile under DRY_RUN. **Status**: Steps 1–4 implemented + verified + merged 2026-06-08 (V15 prod-applied, shared_db v0.15.0 tagged; worker reconcile + Library client; backend sync/state endpoints + contract; front bucket UI). Built via a parallel build→adversarial-review workflow; safety rails baked in (`SPOTIFY_LIBRARY_WRITES_ENABLED=false` plan-only default; never-delete-preexisting via immutable `source` side table; SAVEPOINT per-item PULL isolation). OQ2 resolved: full PULL, pre-existing vs MyBlog-added visually distinguished. **Feature is DORMANT/safe pending 3 OWNER gates**: (a) `terraform apply` to add the POST sync apigateway route (plan clean: 1 add); (b) Step 0 — re-run `scripts/spotify_bootstrap_token.py --write` for `user-library-read`+`user-library-modify` scopes; (c) flip `SPOTIFY_LIBRARY_WRITES_ENABLED=true` after reviewing a plan-only reconcile. Until all three, the worker reads fail-closed (403→needs_attention) and no real Spotify write occurs.
