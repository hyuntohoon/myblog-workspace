...
- **FEAT-music-search-unify-writer** (idea, no RFC) — migrate the writer's `CommandPalette` onto the shared `useMusicSearch` hook (extracted + adopted by the 평론 버킷 `AddAlbumModal` in myblog_front #128, 2026-06-09). Completes the DRY merge: CommandPalette keeps its writer-only UI (artist drill-in, track filter, keyboard nav) but drops its hand-rolled DB/Spotify search + seq-guard (CP-1/CP-4) + cooldown + per-bucket paging duplication. **Risk: 816-line component with a race-condition history → the hook must absorb every edge (in-drill-in search state, per-bucket offsets) without regression; requires a full real-browser click-through re-verify, not just lint/astro-check.** Frontend-only, single PR, no contract change. Fallback if the hook can't cover it cleanly: leave CommandPalette as-is (bucket fix already shipped the user-facing win). Not yet scoped.
