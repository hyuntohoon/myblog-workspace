...
- **FEAT-album-research-notes** (draft) — **auto AI research notes for bucketed albums**: per-bucket opt-in scope (`research_mode` off/all/selected + item checkboxes) → SQS → dedicated research Lambda → Anthropic API (opus 4.8, web_search, album-research-prompt v2) → per-album note in new `album_research` table (UNIQUE(album_id, prompt_version) dedupe; restart **or refine-with-instruction** re-research; writer-GUI-only display — BucketBoard + `/write` — no public exposure; feature-scoped secret `myblog/anthropic`). 5 sequential steps (shared_db V16-ish → infra queue/λ/route → worker handler → backend triggers+contract → front panels). Backfill of ~50 existing items deferred. RFC → `docs/rfcs/FEAT-album-research-notes.md`. Status draft — needs owner accept before Active.
