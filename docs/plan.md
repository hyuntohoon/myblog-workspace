...
- **FEAT-genre-system** (draft, **Step 7 in progress**) — tier-0 genre system: fixed 12-genre English vocabulary (K-Pop = idol-industry; multi-attach), zero-human labeling pipeline (Spotify-string mapping + iTunes UPC anchor + LLM fallback/arbitration), tracks inherit album genres w/ OST/compilation exceptions, public `/genres` map, `/reviews` real-genre filter, `derive_subject_meta` swap. **Steps 0–6 DONE + prod-live** (V17 migration → shared_db v0.17.0/v0.18.1; `genre_mapping.py`; prod backfill 982/982 albums, 1,552 rows, gates pass max share Hip-Hop 49.3%; daily `--incremental` launchd; genres API — public `GET /api/genres/tree` + JWT `PUT /{id}` label/definition/position; `derive_subject_meta` on real `album_genres`; `/reviews` real-genre filter — see `git log` + `docs/archive/`). **Step 7 (this session)** = front `/genres` Outliner page (real tree GET + owner inline edit) + backend `album_count` on the tree node (grouped `album_genres` count) for the share-bar distribution. Cross-repo order: **backend (album_count) → workspace (contract) → front (page)** — additive field, front-graceful. RFC → `docs/rfcs/FEAT-genre-system.md`. **Supersedes FEAT-genre-taxonomy (discarded 2026-06-12 → archive).** ego-view relationships + sub-genre vocab + writer picker = separate **FEAT-genre-subgenres** (Frozen); music-search filter rejected.
