...
- **FEAT-track-play-history** (in flight, code complete) — durable TRACK-level play log. `spotify_play_events` (V10) keeps cumulative listen history at ALBUM grain only; the track detail survived solely in the pruned `spotify_recent_tracks` cache, so no durable per-track history existed. New table `spotify_track_play_events` (append-only, never pruned, dedup `(spotify_track_id, played_at)`); the worker writes it in the SAME tx as the album log, resolving `track_id`/`album_id` from the catalog when known (NULL fail-soft otherwise).
  - **Scope / repos**: `myblog_shared_db` (V19 migration + model + `_generated_schema.sql` + canonical) + `myblog_worker` (`listening_sync_service.py`) + workspace (`docs/contracts/schema.sql`, plan). Write-only — **no backend/music/front, no openapi regen** (worker raw SQL, no shared_db pin bump).
  - **Order**: apply V19 to Neon main **BEFORE** the worker deploy (same-tx INSERT → a missing table fails the whole listening sync). V19 already applied to the Neon **test** branch.
  - **Verification (local)**: worker unit 21/21 + real-engine integration 8/8 (incl. new `test_track_play_events_append_idempotent`: ON CONFLICT idempotency + `tracks.spotify_id` resolution) + shared_db parity/full 52/52 all green. **Prod smoke** (post-deploy): next hourly listening sync logs `track_events=N` and `spotify_track_play_events` gains rows on a new play.
  - **Rollback**: `DROP TABLE spotify_track_play_events;` (V19 down-migration header) + worker revert. No backfill — Spotify exposes only a rolling 50-item window, so history is forward-only from first deploy.
  - **Status**: branches `db/track-play-events` (shared_db) / `feat/track-play-events` (worker + ws); pending **prod V19 apply + worker deploy approval**.
- **FEAT-genre-autoheal** (deferred — behind album-ingest, owner 2026-06-15) — new albums sit genre-less because the daily launchd genre backfill runs `--incremental --execute` which **replays a stale cached `decisions.json`** (06-13 & 06-14 logs identical: `982 albums marked`) and never re-labels → the marker-less 139 newest albums (incl. 45 fully bare) are never enriched. Immediate heal: manual `--incremental --label --execute` (run 2026-06-15, clears them). Durable fix **deferred** per owner: a cloud iTunes-baseline heal hung **behind the daily album-ingest batch** (worker EventBridge), keeping the local launchd LLM pass as the low→high / K-Pop-arbitration upgrade. **Blocker for full cloud**: the S3 LLM step shells `claude -p` on the Max subscription token (`~/.claude/.credentials.json`), absent in Lambda → cloud pass is iTunes-only; must use a DISTINCT partial marker so it never excludes the LLM pass. Not scoped/accepted.
