...
- **FEAT-album-research-notes** (**accepted 2026-06-11**, in progress) — per-bucket opt-in auto AI research notes (scope modes, dedicated researchSQS/Lambda, opus 4.8 + web_search, `album_research` table, restart/refine, writer-GUI display in BucketBoard + `/write` gutter). RFC → `docs/rfcs/FEAT-album-research-notes.md`.
  - **Step 1 (shared_db) MERGED** — V16 `album_research` + bucket `research_mode`/`research_selected`, shared_db **v0.16.0** (PR #25). Applied to Neon **test** branch; **prod V16 apply still pending** (rule #3 — gates Step 3/4 merges).
  - **Step 2 (infra) MERGED** — researchSQS + DLQ, researchWorkerLambda (900s, reserved-concurrency 2), `myblog/anthropic` secret container, research-worker IAM (workspace PR #304, `plan` 7-add/0-change). **`terraform apply` NOT yet run** (no infra auto-apply — owner manual).
  - **Owner-manual gates before Step 3**: (a) `terraform apply` the infra; (b) create Anthropic API key in console → set `ANTHROPIC_API_KEY` in `myblog/anthropic`; (c) apply V16 to **prod** Neon.
  - **Next: Step 3 (worker handler + 1 real cost-measured run)** → 4 backend (hook + routes + API GW route + contract) → 5 front. Step 3 note: worker CI must deploy **both** blogWorkerLambda and researchWorkerLambda from here on.
