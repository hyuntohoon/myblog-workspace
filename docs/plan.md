...
- **STAB-4** (draft) — **schema.sql backfill + doc drift** (STAB-1 §Seq item 4 / P2-8, MODEL-9, MODEL-12, P2-1/2/3/4, P1-7). **Doc-only.** Prod introspection (06-05): 6 tables missing from `schema.sql`, `posts.rating` stale `(3,1)` vs prod `(3,2)`; `library_items` (V7) is a **ghost** (absent in prod) → backfill from `pg_dump`, not migrations/ORM. RFC → `docs/rfcs/STAB-4-schema-and-doc-drift.md`. Status **accepted** (2026-06-05, owner-approved).
