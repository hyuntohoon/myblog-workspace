...
- **Status**: done (2026-07-04 — all 4 steps shipped same day; Step 4 front #230, prod-smoked)
- **Owner**: 박지훈
- **Created**: 2026-07-04
...
| 2026-07-04 | Step 4 executed — front #230 (viewer 번역 toggle + 번역 요청 button lifecycle + Hangul-ratio ≥50% Korean-dominant hide + `manual` origin badge; identical-passthrough `text_ko` lines not duplicated). Pre-merge: lint/astro check 0 errors + real-browser CDP (toggle interleave 43 ko lines on Berghain, nav unchanged, Korean-dominant hidden, POST fires once → 요청됨; test row deleted pre-claim). Prod smoke post-deploy: bundle marker + real-browser toggle/nav + full suite 28/0. **RFC complete → Status done, archived** | 4 |
