...
| 2026-07-13 | **Step 5 SHIPPED + prod-smoked** (worker #71, deploy 29214115447): confirm-on-ingest (`release_confirm_service`, artist + normalized-title + ±7d, all-source flip / spotify-source insert, IS-DISTINCT-FROM idempotency guards) + OQ5 (floor 60→50, watch-scoped `include_groups=album,single`) + MB budget fairness (stateless intra-bucket offset rotation) + `no_upc` sentinel retry-gate bypass. 374→378 pytest. First live invoke: eligible=1593 swept=30 confirm_inserted=103. `SWEEP_ARTISTS_PER_TICK` stays 30 (owner 관망; ~51 d rotation ⇒ confirm lag flagged) | Step 5 |
| 2026-07-13 | **First-live-tick incident + same-session fix (worker #72)**: the never-announced confirm INSERT bypassed the ingest `ALBUM_POP_MIN` gate → 103 rows incl. Schubert-class credit-farm compilations hit the calendar from one 30-artist sweep. Fix (owner-approved immediately): candidates carry `passes_gate` (external-phase popularity fetch, novel-probe reuse, fail-closed), INSERTs gated, flips exempt (MB/iTunes observation = curation), new `confirm_gate_skipped` counter. One-time cleanup DELETE of ungated rows (catalog-membership criterion; dry-run 48). **Known residual for OQ4**: comps scoring ≥20 pass the bar (35 in-catalog Schubert rows survive) — evaluate with `confirm_gate_skipped` data at the 14-day review | Step 5 |
| 2026-07-13 | **Step 7 SHIPPED + prod-smoked** (front #273, deploy 29214166053): `/releases/` — ledger default + month-grid toggle + day-cell card panel per the mockup gate; `?view=&month=` URL state; 93 d cap respected (one month per fetch); NewReleasesCard 캘린더 → link (degradation contract intact); released rows resolve via `GET /albums/by-spotify/{id}` → openAlbum. lint/astro check/build + live-data CDP both views + mobile; reviewer merge-with-nits (nits logged in PR) | Step 7 |
| 2026-07-14 | **Mid-window ops health check (read-only, DB evidence): HEALTHY.** All three write paths alive today (MB last insert 07:02 UTC, iTunes upserts touching rows, confirm tick 05:32 UTC); post-#72 gate leak = 0 (22/37 post-fix confirm rows join catalog at pop≥20 min, 15 await catalog sync — re-check ~24 h whether confirm outruns catalog); dup keys / NULL dates / absurd dates all 0. **Watch items**: (1) zero announced→released flips have EVER fired in prod — exercise deliberately if a known watchlist release day passes without one; (2) `no_upc` sentinels grew 312→492 (30.6% of watchlist) — the owner-approved one-time clear is still pending; (3) pre-#72 residue (2 pop-0 + 21 album-less rows) → fold into the OQ4 noise tally rather than delete now. OQ4 preview: review-window release_date rows = 20 (14 album / 6 single, 30% singles); formal review ~07-27 with `confirm_gate_skipped` counter | ops |
| 2026-07-14 | **Sentinel-clear ops EXECUTED — narrowed from the owner-approved DELETE (BEGIN…COMMIT, in-txn verified).** The PR-#71 SQL targeted all 820 `source_artist_id='not_found'` iTunes rows lacking a UPC-bearing catalog album. Split by `resolved_via`: 332 `not_found` (legacy pre-Step-5, stuck behind the 30 d retry gate) + 488 `no_upc` (Step-5-era, already retry-gate-**bypassed** — a widened-catalog UPC re-resolves them at the next bucket visit with zero HTTP). Deleted **only the 332**; leaving the 488 avoids pure write-churn re-creation of sentinels the poller already rechecks every cycle. Post-state: iTunes `not_found` 459→127, `no_upc` 492 unchanged, `upc` 652 unchanged. The widened catalog now re-resolves the 332 at their next bucket visit instead of ≤30 d later | ops |
