...
>
> **Known cost, recorded not hidden**: with the viewer open on the dashboard, one transport command now causes **two** reads — `NowPlaying`'s pre-existing 1:1 confirmation read plus the viewer's. Attributed by closing the viewer and re-measuring (2 → 1). Both are 1:1 with a user action, so it is not a storm; it is a natural dedupe target once `FEAT-lyrics-viewer-playback` gives the viewer its own transport buttons. **Updated 2026-08-03 (OQ4, front #342)**: the dedupe went the other way. Making `sendPlayerCommand` dispatch would have made it *three* reads, so NowPlaying now ignores events raised by its own commands (`controlBusyRef`) — its own commands stay at one read, and it gains a read only for commands issued elsewhere, which is the read it was missing. The viewer's ⏸/▶ costs one read where it previously cost none; that read is what puts a transport command into this series at all.
...
| 2026-08-01 | Two reads per transport command while the viewer is open (`NowPlaying`'s + the viewer's) accepted as-is and recorded, rather than deduped now — both are 1:1 with a user action, and `FEAT-lyrics-viewer-playback` is the natural place to collapse them. **Resolved there 2026-08-03 (OQ4)**: not by collapsing the two, but by scoping each listener to the commands it did not issue. | 2 |
