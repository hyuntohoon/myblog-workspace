...
- **Status**: done (closed 2026-07-06 with explicit owner approval — all 6 planned steps + the 2026-06-25 discoverability/rendering follow-ons + the 2026-07-06 member-authoring follow-on shipped + prod-verified; the residual "future kinds" were hypothetical seams, not committed scope. Promoted draft→in-progress 2026-06-24; design decided via the Design Atlas + configurator, OQ 1–5 resolved as a user-selectable design setting)
- **Owner**: TBD
- **Created**: 2026-06-23
...
- **D3 — Playback aims for real playback; v1 = Spotify Premium.** The Playback bucket aims for **actual
  ...
  **YouTube fallback is DEFERRED** (kept possible, not built — its ToS forbids background/audio-only and
  ...
  > **CORRECTED 2026-09-05 (`FEAT-youtube-playback-provider` Step A1):** two of the claims above are false and were load-bearing for fifteen months. (1) **"Membership stores a provider-neutral track identity (ISRC / artist+title) resolved to a provider id at play time" never shipped.** Playback membership carries `review_bucket_items.track_id`, a foreign key to `tracks.id`; all 29 production playback rows have it. There is no ISRC-or-artist+title resolution step — which is false *in our favour*: there is nothing to unwind in order to add a second provider. (2) **The "~34% wrong-version" rate is unsourced and does not hold on this catalog.** Measured 2026-09-05 on a 20-track reproducible sample of our own `tracks` plus 2 controls: correct studio version **20/20**, wrong-version **0**, `status.embeddable` **20/20**, both controls passing. That figure was the load-bearing justification for deferring YouTube across three RFCs. The *policy* objections in this bullet (no background/audio-only playback, a visible ≥200×200 player) remain true and still shape the design.
...
> **CORRECTED 2026-09-05 (`FEAT-youtube-playback-provider` Step A1):** three claims in the paragraph above did not survive measurement. (1) **Membership does NOT store a provider-neutral identity** — it stores `review_bucket_items.track_id` → `tracks.id`, on every one of the 29 production playback rows. (2) **"~34% wrong-version" is unsourced and false on this catalog** — 20/20 correct, 0 wrong-version, 20/20 embeddable, measured 2026-09-05 with controls. (3) **"`search.list` costs 100 units against a 10k/day quota" is no longer how Google prices it** — `search.list` now sits in its OWN bucket at **100 calls/day, 1 unit each**, so it no longer draws on the 10,000-unit pool. The ceiling is unchanged but the consequence is not: a `videos.list` maintenance job (1 unit per 50 ids) now runs essentially free beside it, which is what makes a *refreshed* mapping table viable at all. The old arithmetic reached the right conclusion for the wrong reason and would have mispriced the maintenance job. **Still true and still binding**: no background or audio-only playback, a visible ≥200×200 player, and no ISRC search on public YouTube. A queue is more session-like than a stored playlist (a "Save queue to playlist" bridge — internal,
...
| Date | Decision | Step |
|------|----------|------|
| 2026-07-06 | **Member-authoring follow-on DONE + merged** (myblog_front PR **#244** `be35d06`) — the "remaining future kinds" seams closed: front creation/add paths for **review** (평론 담기 on the review hero, `review_target_id` = frontmatter `postId`; logged-out `pb:resume` intent union extended to review — playback/snapshot deliberately not stashed), **playback** (queue-mode toggle on the track picker sheet, `item_type='playback'`, D8 dup-allowed), **snapshot** (스냅샷 담기 on the 분석 버킷 freezing the displayed metric/period/totals into a `SnapshotCaptureRequest`). Tiles render without a contract change: playback reuses the serializer's `TrackBrief`; review/snapshot titles come from an add-time `note` caption **scoped to those kinds** (reviewer should-fix — an album row's user memo must never surface as a title). Plus `AddToBucketMenu` → `bucketStore.ensureFresh(true)` after add/undo, fixing the pre-existing ≤5-min board/tray lag behind menu adds (all kinds). Front-only; no route/infra change. Verified: lint + astro check 0 err; real-browser CDP vs prod API (review 201+note caption / dup 409 / 2 playback dups + 1 deduped track / snapshot 201 with `bucket_item_snapshots` side-row `kind=period, total, source_album_ids[12], frozen`, count+time captions; board tiles all labeled; no-reload board update; console 0; temp bucket deleted, zero prod residue); 3-lens project reviewer 0 blockers. With this, **every planned member kind is authorable + renderable** — no concrete open items remain in this RFC. | follow-on |
| 2026-07-06 | **RFC CLOSED (owner approval, same session as the authoring follow-on).** Everything committed in *Steps* is prod-live: front design layer (all shells selectable), V28–V31 generalized membership + snapshot side-table + `들을 것` fold, item_type-branching backend + append-only snapshot capture + the dormant single-owner playback token (later activated by FEAT-spotify-streaming-playback), 5a/5b item types + sign-in resume + lazy playback SDK, Step-6 relax + track-as-member + `들을 것` kind-cutover, and the #211/#212/#244 follow-ons. Deferred-by-design (documented seams, deliberately NOT this RFC's scope): per-listener streaming OAuth + multi-user scoping (`FEAT-multi-user-accounts`), Summary-bucket auto-processing (D4 — no summary backend exists; a future RFC if wanted), YouTube provider fallback, Spotify extended-quota request before any multi-user launch. Status → done, archived to `docs/archive/done/rfcs/`, plan row dropped (compact trail entry kept). | — |
