...
- **Status**: done (all steps 0–3 executed + prod-verified 2026-07-03; closed + archived 2026-07-04, owner-approved in-session)
- **Owner**: 박지훈
- **Created**: 2026-07-03
- **Plan row**: dropped on archive 2026-07-04 (history: `docs/archive/done/2026-07.md` + `git log`)
...
| 2026-07-03 | **Step 2 implemented** (worker `92c9ed8`, explicit owner go "step2 go") — all four coordinated changes: (1) `run_eval_batch` invokes `promote_best` after `decide_match` on `ambiguous`/`review_required` only, before the invariant + writer, reusing the same in-memory candidates (no second LRCLIB fetch); (2) consistency invariant basis-aware (`_consistency_ok`: exact-title ⇒ version agreement, `best-of-*` ⇒ `evidence.promotion` present — a stray best-of `matched` without provenance is refused); (3) ladder reshuffled to integer rungs `{None:0, fuzzy-title:1, best-of-ambiguous:2, best-of-review:2, exact-title:3, mb-recording:4, isrc:5}` — both best-of bases share rung 2 so strict-`>` gives exact-supersedes-best-of / lateral-refusal / never-displaces-exact for free; (4) reassessment selection widened (`OR matched AND match_basis LIKE 'best-of-%'`) with unresolved-first `CASE` ordering. Metrics: `promoted_ambiguous` / `promoted_review` / `promotion_parked` / `promotion_criteria` per-criterion map. 13 new tests (promotion-through-the-loop incl. Come Back to Earth end-to-end, review-sibling path, stray-best-of refusal, best-of ladder, widened-SQL assertion); full worker `pytest` 275 passed / 34 skipped. **Known wrinkle (accepted)**: a re-selected best-of row whose re-check yields the same-strength outcome is `guard_kept` without an `updated_at` bump, so the best-of tier itself doesn't rotate — unreachable in practice while the unresolved pool (~10k `not_found`) dominates the LIMIT-150 selection, and self-correcting on supersession. Merged as worker **#61** (`1e7510f`), deploy run 28659357669 green. **Prod smoke PASS** (manual `lyrics_reassessment` invoke, DB-state evidence — prod `LOG_LEVEL=WARNING`): first live run promoted **32** rows (`best-of-ambiguous`, `matcher_version=step2-v2+best-of-v1`) — ambiguous 2,755→2,723 (−32), matched 7,957→7,988 (+31), no_lyrics 444→445 (+1 all-instrumental), deltas reconcile exactly; **exact-title matched 7,957 untouched** (regression guard); best-of rows with empty lyric text = 0 (body precondition); sample rows carry the full `evidence.promotion` block + preserved `multiple_plausible` original evidence + `version_agrees=true` (result quoted on worker #61). Next: Step 3 (retroactive local one-shot backfill of the remaining ~2,723, OQ4) — separate session per hard rule #4. | 2 |
| 2026-07-03 | **Step 3 complete — retroactive backfill run against prod** (explicit owner go "step3 go"; local one-shot per OQ4, driver `tools/lyrics_bestof_backfill.py` on the workspace repo — a thin chunked wrapper that reuses the worker's live path verbatim: `run_eval_batch` → `decide_match` → `promote_best` → basis-aware invariant → `should_replace` → `TrackLyricsWriter`, per-row commit; pre-run rollback snapshot of all parked rows + `updated_at`-based retry passes scoped to the run-start pool; LRCLIB @40 concurrency). **Session baseline (12:27 UTC): ambiguous 2,723 + review_required 34 = 2,757 parked.** 20-row sanity slice first (60 rows written — a since-fixed retry-loop artifact pulled the next slice per pass; all writes valid promotions, verified in prod: `matcher_version=step2-v2+best-of-v1`, `evidence.promotion` + preserved `multiple_plausible`, `version_agrees=true`, non-empty bodies), then the full 2,697-row run (16.8 min, evaluated 2,679 = 99.3%, `consistency_errors` 0, `guard_kept` 0). **Session deltas (before→after, reconcile exactly):** ambiguous 2,723→**18** (all 18 = LRCLIB transient skips after 2 retry passes, rows untouched → drain via reassessment); review 34→**26** (26 parked with recorded attempt — no version-agreeing sibling, the OQ5 follow-on class; 8/34 promoted, exactly the probe's prediction); matched `best-of-ambiguous` 31→2,603, `best-of-review` 0→8 (**+2,580 best-of matched**); `no_lyrics` best-of 1→126 (+125 all-instrumental); `exact-title` 7,957→7,965 (+8 = pool-drift rows freshly resolving as full exact matches, all 8 verified members of the run-start pool; **zero pre-existing exact-title rows touched** — regression guard). **Coverage gain 2,705/2,757 = 98.1%** of the parked pool promoted via best-of (98.4% incl. drift); corpus `matched` share 37%→**49.5%** (10,576/21,366). Tier breakdown 100% `exact-base-title` (v1 tier-1-only by construction); best-of rows with empty lyric 0; missing `evidence.promotion` 0. **Manual precision audit: stratified random 40/2,705 promotions (all 8 best-of-review + 32 random ambiguous; synced 37 / plain 3, EN/KR/JP mixed) → 40/40 correct = 100%**, ≥90% bar met (OQ1). Snapshot/report/audit artifacts in `tools/out/` (gitignored). All RFC steps (0–3) now executed; acceptance criteria all verified. | 3 |
| 2026-07-04 | Status in-progress → **done** + RFC archived to `docs/archive/done/rfcs/` (explicit owner approval "아카이브 ㄱ"). All steps 0–3 executed + prod-verified; plan row dropped; follow-on = OQ5 matcher noise source-fix RFC (26 review rows + false-ambiguity shrink at the source) | — |
