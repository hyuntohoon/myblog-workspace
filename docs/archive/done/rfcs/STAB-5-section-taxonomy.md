...
### Step 4 — review tags via `tags`/`post_tags` — **DONE ✅ 2026-06-06 (backend + contract + writer picker)**
...
**DONE (Lane B — writer tag picker, front #101 `fba6118`, deployed):** build-time seed `src/lib/tags.ts` (5 tags, lockstep with the prod seed) + a read-only multi-select chip group in the writer's 발행 설정 panel (`SettingsPanel`) + `tags` state in `WriterApp` (seeded from `post.tags` on edit-load and from the autosave draft, **sanitized to the seeded vocab** to guard a stale legacy `tags` key) wired into the `/api/posts` create+update payloads (**DB-only** per D5; not the `/api/publish` MDX call). No contract/`api.gen.ts` change (already synced). **`onToggleTag` uses a functional state updater** so rapid successive toggles compose instead of clobbering a stale closure — **this exact bug was caught by the browser click-through (lint + astro-check missed it)**, per the BUG-11→BUG-12 lesson.
**Verification:** `pnpm lint` 0 / `astro check` 0 errors / `pnpm build` OK (Node 20.19.5). **Headless CDP click-through (local + prod `/write`):** 5 chips render with the right labels, multi-select toggles compose, toggle-off works, selection persists to the autosave draft; prod confirmed the deployed bundle renders + toggles. Backend write-path already prod-verified (smoke-JWT e2e: attach/replace/clear/reject-unknown-400). 
...
| 2026-06-06 | **Step 4 writer tag picker DONE (front #101 `fba6118`, deployed) — STAB-5 now FULLY IMPLEMENTED (all steps 1–6 done).** Build-time seed `src/lib/tags.ts` + read-only multi-select chips in 발행 설정; `tags` wired into `/api/posts` only (D5). **Bug caught by the click-through:** `toggleTag` read the `tags` prop directly → two rapid toggles shared a stale closure and clobbered each other; fixed with a functional `setTags(prev => …)` updater (lint + astro-check both passed the buggy version — BUG-11→BUG-12 lesson reconfirmed). Verified local + prod CDP click-through (5 chips, compose, persist). **RFC archive + plan-row drop pending explicit owner approval (rule #5 — status/archive is human-gated).** | Step 4 |
