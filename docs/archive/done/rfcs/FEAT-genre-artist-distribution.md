...
> **✅ Post-merge hardening (2026-06-21).** Found while verifying classify on prod: (1) `POST /api/library/saved-tracks/classify` **404'd** — POST routes need an explicit `infra/apigateway.tf` entry (GET rides the `/api/{proxy+}` catch-all); added + applied. (2) `send_album_sync` sent all 712 ids in ONE SQS message → the worker's 120s Lambda timed out; **chunked to 20/msg** (worker #80). (3) the burst **livelocked** `AlbumSyncService`'s artist upsert (dict-order locking → deadlock under the account's 10-way Lambda concurrency); **sorted by `spotify_id`** (worker #48). (4) classify catalogs albums but the cache `album_id` re-resolves only on a saved-tracks **full** sync — so the EventBridge crons (daily incremental + weekly full) were **applied** (the previously-deferred infra). **End state: 분류하기 cleared all 712 uncatalogued + a genre backfill cleared the 264 ungenred → 미분류 1,017 → 0.** The 분류하기 button later **merged with the on-demand `장르 채우기` trigger** (see `FEAT-genre-autoheal` in `2026-06.md`) into ONE button doing catalog-sync + genre-backfill. NB the by-artist/by-genre *grouping view* the owner originally meant lives in the **평론 버킷** (`FEAT-bucket-organize`), not here — the 분석 버킷 buttons are data-fillers, not a view.
