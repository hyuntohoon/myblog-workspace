...
- `/reviews` 인덱스만 `genres[]` 배열 인식 필터 보유(`myblog_front/src/lib/reviews.ts`); music search 엔
  장르 필터 없음.
- shared_db 최신 마이그레이션 **V15**(spotify_library_sync, FEAT-spotify-library-sync), 버전 **0.15.0**; backend pin `@v0.15.0` (동기). genres 는 다음 연속 번호 **V16** (V14 `spotify_recent_tracks` + V15 `spotify_library_sync` 가 선점; un-defer 시 재확인). _(갱신 2026-06-09; Step 1/2 본문의 잠정 pin 번호는 decisions log 대로 un-defer 시 재산정.)_
- 관리형 admin write 의 정착 패턴: backend 의 Cognito-JWT 라우트(`posts`/`publish`/`buckets`).
  단일 사용자(주인장) — user 테이블·author_id 없음.

## Target state

### 데이터 모델 (shared_db, V16)

`migrations/V16__genres.sql` + `models.py` `Genre` 모델. 단일 사용자 → `user_id` 없음.

> **마이그레이션 번호 (2026-06-09, re-resolved)**: genre 는 **DEFERRED** 라 코드에 마이그레이션 파일이 없으므로
> active 작업들이 번호를 먼저 가져간다. V12 다음 *실제로 적용되는* 순서 = STAB-5 (sections rename+seed) **V13** →
> `FEAT-member-dashboard-realdata` (`spotify_recent_tracks`) **V14** → `FEAT-spotify-library-sync` (`spotify_library_sync`) **V15** → genres = **V16** (다음 연속 번호). deferred
> 동안 번호를 예약하면 그게 곧 genre RFC 가 금지하는 마이그레이션 갭이 되므로 항상 active 작업에 양보하고 genres 는
> next-free 를 쓴다. 주인장 승인 2026-06-06(V13→V14) → 2026-06-08(V14→V15) → 2026-06-09(V15→V16).
...
### Step 1 — shared_db V16 마이그레이션 + Genre 모델

`migrations/V16__genres.sql`(위 DDL), `models.py` `Genre`(self-ref `parent`/`children` + 2단 다운그레이드
주석), `_generated_schema.sql` regen, `pyproject.toml` 버전 bump + 태그 (un-defer 시점의 다음 minor;
번호는 V15/0.15.0 (spotify-library-sync) 까지 적용된 *이후* 기준으로 재산정), canonical `docs/contracts/schema.sql` 동기.

**롤아웃 (CRITICAL — `reference-shared-db-cross-repo-rollout`)**: V16 를 **prod Neon 에 머지 *전* 적용**
→ `\dt genres` 확인 → shared_db PR 머지 → 태그. 순서 틀리면 Step 3 backend 가 unknown-table 500.
V16 는 STAB-5 V13 + member-dashboard-realdata V14 + spotify-library-sync V15 적용 이후 다음 연속 적용.

**Verification**:
```
cd myblog_shared_db && pytest        # 모델 ↔ _generated_schema.sql 일치
psql "$TEST_DB_URL" -f migrations/V16__genres.sql -v ON_ERROR_STOP=1   # 클린 적용
...
`requirements.txt` pin = Step 1 의 genres 태그 (un-defer 시 재산정; 현재 backend `@v0.15.0` → genres 는 그 다음 minor; 중간 V13 section / V14 spotify_recent_tracks / V15 spotify_library 는 genre 무관 슬롯), `infra/apigateway.tf` `genres_post`(JWT, **buckets_post 복사**),
...
2. **`/genres` 경로 (blocks Step 5)** — `/genres` vs `/admin/genres` vs `/reviews/genres`. v1 은
   로그인 후에만 접근. 기본 `/genres` 제안. **(2026-06-05: 경로 결정 보류 — 주인장이 기능 착수 시점에
   확정. 기능 자체가 추후 작업으로 파킹됨, `plan.md` Later 섹션 참조.)**
...
| 2026-06-08 | **genres 마이그레이션 V14→V15 재지정**: `FEAT-member-dashboard-realdata` 가 V14 를 선점 (shared_db `V14__spotify_recent_tracks.sql` + v0.14.0) → deferred genre 는 next-free **V15**. plan.md genre row 동기. 버전/pin 은 여전히 un-defer 시 재산정. **(→ 2026-06-09 V15→V16 재재지정, 아래)** | — |
| 2026-06-09 | **genres 마이그레이션 V15→V16 재지정**: `FEAT-spotify-library-sync` 가 V15 를 선점 (shared_db `V15__spotify_library_sync.sql` + v0.15.0, 2026-06-08 prod-applied/머지) → deferred genre 는 next-free **V16**. plan.md genre row + rfcs/README.md 동기. V15 마이그레이션 헤더 자체가 "genre re-bumps to V16" 명시. 버전/pin 은 여전히 un-defer 시 재산정. | — |
