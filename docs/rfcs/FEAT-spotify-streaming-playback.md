...
1. **Premium account identity** — which Spotify account holds the owner's catalog/library, and is it Premium? The `streaming` consent in Step 1 must be granted by a Premium account, and it is the account that will actually stream. *Blocks Step 1 + Step 4.*
