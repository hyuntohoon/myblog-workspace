...
2. ~~**Resolve shape (2a vs 2b vs both)**~~ **RESOLVED 2026-06-25**: (b) a `/resolve` endpoint, **uniform** for both ▶ sites (both pass only a DB id; the review tracklist has no brief at all). Either shape needs a contract change + `api.gen.ts` regen, so `/resolve` wins on blast radius — it doesn't widen the broadly-reused `AlbumBrief`/`TrackBrief` or risk leaking a streaming handle onto the public projection. Brief-carried (a) deferred as a future tray optimization. Auth = **edge_guard-only** (not JWT): `spotify_id` is a public identifier and the catalog is otherwise edge_guard-only → no Cognito JWT, no `infra/apigateway.tf` route, **no terraform apply**.
3. ~~**Token re-mint strategy**~~ **RESOLVED 2026-06-25**: `getOAuthToken` re-mints via `getStreamingToken()` on every SDK callback (the SDK invokes it near expiry / on 401). `getStreamingToken` caches with a 5s skew so steady-state calls are cheap, with a fallback to the initial token if a re-mint fails.
4. ~~**Album playback semantics**~~ **RESOLVED 2026-06-25**: an album ▶ plays as a `context_uri` (full-album context, `spotify:album:<id>`); a track ▶ as a `uris[]` entry (`spotify:track:<id>`).
5. ~~**Region/availability**~~ **RESOLVED 2026-06-25**: a 403/404 from `PUT /me/player/play` surfaces a clear "이 트랙은 현재 계정/지역에서 재생할 수 없어요." notice (no skip-to-next in v1).
