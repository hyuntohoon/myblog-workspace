...
### Step 7 — E1: `?explain=1` response mode ✅ DONE (2026-06-05)

> ✅ Done 2026-06-05 — music `#38` (`dfec791`) → workspace contract `#240` →
> front `#91` (`d64d93f`). All three deployed; front types-in-sync gate green.
>
> **Deviation from the step text (intentional):** the step described wrapping
> each row as `{ row, debug }`. Implemented instead as a single additive
> optional top-level field `UnifiedSearchResult.debug: List[ExplainEntry] | None`
> (one entry per returned row: `bucket, id, rank, path, matched_field,
> similarity, popularity`). This keeps the **default response shape and every
> item schema unchanged** (debug omitted without `?explain=1`) and is trivially
> typed end-to-end through the merged OpenAPI → front `api.gen.ts`. A per-item
> `{row, debug}` wrapper would have been a breaking shape change across all
> three buckets for a dev-only triage surface — not worth it (necessity gate).
> `similarity` is the service-layer bucket score (0–3) used in ranking, not the
> raw pg_trgm float.
>
> Verification: pyright 0 errors; `pytest -m "not integration"` 40 passed; new
> integration test `test_explain_mode_attaches_per_row_debug` (debug None by
> default; one aligned entry per row under `explain=True`). **Prod smoke**:
> default response has no `debug` key; `?explain=1` on `방탄소년단 Proof`
> returns `debug=[{bucket:album, rank:1, path:decomposed, matched_field:title,
> similarity:3.0}]` — also re-confirming Step 6 decomposition is the surfacing
> path. Contract propagated cleanly (auto-PR #240 diff = ExplainEntry + debug +
> explain param only); front regen +21 lines, lint + astro check clean.
