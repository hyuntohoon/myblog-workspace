...
4. **Artwork contract shape (Step 3)** — extend `Backend_TrackBrief` with `cover_url` (recommended, see
   *Data/API/contract impact*) vs. surface `tracks.spotify_id` on playback rows for a client-side
   Spotify-catalog resolve. Low ambiguity — the recommendation is strong — but recorded as a decision
   point since it's the one contract choice this RFC makes.
