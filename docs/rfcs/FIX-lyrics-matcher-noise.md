...
| 2026-07-04 | **Probe-driven scope revisions** (2 old-vs-new probe rounds caught 3 regression classes before any merge): (1) noise strip extended to **trailing non-rendition dash segments** (`- From "Movie"`, `- 2013 Mix` — the Spotify spelling of the same retitle noise; catalog-side dash vs candidate-side paren split asymmetrically); (2) **group key = base title only** (artist component split identity-passing spelling/alias variants — BTS `방탄소년단` class — into false ambiguity; every plausible candidate already passed the identity gate); (3) **`exact_base_equal` = noise-stripped OR plain (step2-v2) canonical equality** — the new exact set is a strict superset of the old, so no pre-fix exact match can be lost to an over-eager strip (`Doo Wop Freestyle ('99)`: the parenthetical year IS the title) — and the **fuzzy gate runs on PLAIN canonicals only** so the strip never pulls a different longer title (`Booty Out the Window`) into fuzzy range; (4) representative gains a **full-canonical-equality preference** for pick stability | 1 |
| 2026-07-04 | **Step 1 DONE** (worker branch `fix/lyrics-matcher-noise`, 3 commits `d1e2d7d`/`6710862`/`474ce93`; 308 passed / 34 skipped). Read-only probe `tools/lyrics_matcher_noise_probe.py` (old step2-v2 loaded from `git show origin/main`, new from branch, SAME re-fetched candidates — code effect isolated from LRCLIB drift; pools: unresolved 44 / best-of sample 381 / exact-title sample 300): **(ii) exact regression guard 0 lost, 300/300 (100%) same pick** (rounds 1–2 had 9→2 losses, fixed at source, round 3 clean); **(i) source resolution**: old-ambiguous → new matched at decide level 272/397 (68.5%), rest keeps resolving via the existing best-of promotion; changed-outcome universe 275 rows, all `exact-title`; NEWLY-ambiguous 3 = review→ambiguous **lateral parks** (no write risk); **(iv) labelled precision audit 68/68 = 100%** (denominator: stratified sample of the 275 changed rows — track+candidate 43 / candidate-only 2 / rep-only 23; KR covered), ≥90% gate (OQ3) **PASS**; unresolved-44 preview: 23 → matched, 21 stay review. Artifacts: `tools/out/noise_probe_report.json` + `noise_probe_checkpoint.jsonl` + `noise_probe_audit_sample.json` (gitignored) | 1 |
| 2026-07-04 | **Step 2 DONE + prod-smoked** — worker PR #64 squash-merged (`c53c48a`), deploy run 28692610239 green. Manual `lyrics_reassessment` invoke (DB-state evidence, prod `LOG_LEVEL=WARNING`): 148 rows touched, **matched 10,628→10,632 (+4 = 3 ex-ambiguous + 1 ex-not_found, deltas exact)**, all 4 `exact-title`/`step3-v1`/non-empty bodies; pre-existing exact-title 8,001 untouched; best-of 2,619/8 unchanged (result quoted on worker #64) | 2 |
| 2026-07-04 | **Step 3 DONE** — pool-scoped one-shot over the 41 residual rows (`tools/lyrics_bestof_backfill.py` reusing the live step3-v1 path verbatim; pre-run snapshot kept, yesterday's best-of artifacts preserved as `.bestof-20260703.bak`): **evaluated 41/41, matched +20 (source exact-title +16, best-of-ambiguous +4), ambiguous 15→0, review_required 26→21**, consistency_errors 0, unresolved skips 0, 0.4 min. Close-out vs the RFC targets (denominators: 26 review + 18 ambiguous parked as of 2026-07-04 morning): ambiguous residual fully drained (18→0 via smoke+one-shot), review 26→21 (the 21 have no tier-1/version-agreeing candidate even post-fix — honest conservative residual); corpus matched 10,628→10,652, exact-title 8,001→8,021, false-ambiguity inflow now resolves at source (probe: 68.5% of old-ambiguous at decide level, remainder via existing promotion). Status → **done**. Week-1 inflow ambiguous-rate observation vs the 40:1 baseline left to normal reassessment metrics | 3 |
