...
| 2026-08-06 | **Erratum — Step 5's closing header overclaims at the per-slot-pairing level.** A 2026-08-06 evidence-based audit (`ARCH-album-card-contract-and-composition.md`, filed as a follow-up RFC) verified the ninth slice's own claim — "E4's original `play`/`add`/`drag` slot set is now granted somewhere sensible across all three TrackRow surface groups" — against current code. It holds at the surface-group level, but not at the per-slot-pairing level for the `AlbumDetail modals` group: `StandardModal` has `add`+`drag` paired (correct, per Rule #14); `MemoWindow` (front #368, this slice) has `drag` alone, with no tap-based `add` alternative — the one call site in the app where a drag grant shipped unpaired against this RFC's own binding rule. `component-map.md`'s "Ownership by domain" table (`openLyrics?+drag`) already recorded `MemoWindow`'s actual capability set correctly; only this Step's closing header's aggregate framing read as stronger than the per-surface facts support. Tracked as BUG-24 (`plan.md`, independent of this closed RFC); fix scoped to the follow-up RFC's Stage 7, not reopened here. No other claim in Step 5 was found incorrect | 5 |
