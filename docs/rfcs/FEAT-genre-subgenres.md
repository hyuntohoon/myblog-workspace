...
3. **Related-review recommendation** — from a review, recommend connected reviews via shared sub-genres + the sub-genre relationship graph (influenced-by / related edges). **The edge graph is the recommendation engine.**
...
- **Reviews** live in the `blog` content collection (build-time MDX). `musicReview.genres[]` in frontmatter is the *machine* `album_genres` copy — there is no editorial review↔sub-genre link yet.
...
- **New `genre_edges`** — the relationship layer (the ego-view source AND the recommendation engine):
  ```
  genre_edges(
    from_genre_id  UUID REFERENCES genres(id) ON DELETE CASCADE,
    to_genre_id    UUID REFERENCES genres(id) ON DELETE CASCADE,
    type           TEXT CHECK (type IN ('parent','influenced_by','related')),
...
- **New `post_genres`** — the editorial review↔sub-genre link, per-post, separate from `album_genres`:
  ```
  post_genres(post_slug TEXT, genre_id UUID REFERENCES genres(id) ON DELETE CASCADE, PRIMARY KEY (post_slug, genre_id))
  ```
  (Keyed by post slug — the `blog` collection has no DB post row; see Open questions.)

### Vocabulary — a rich seeded palette, organically displayed

- **Palette ≈ 220 sub-genres** (Appendix A), seed-built up front from credible, real-site-sourced taxonomies (RYM / Wikipedia / AllMusic / Discogs / Pitchfork). This is the **writer's pick-list**, optimized for *tagging precision*, not browse leanness.
...
### Step 4 — `/genres` ego-view + populated Outliner (front)
Extend `/genres`: Outliner shows populated sub-nodes; clicking any node opens the ego-view local diagram from `genre_edges`. Reuse the deferred design layouts.

### Step 5 — related-review recommendation (front + backend)
"Related reviews" block on `/review/{slug}`: shared-sub-genre reviews + edge-linked reviews, ranked. Built-time (content collection) or a light endpoint — decide at execution.

### Step 6 — owner edge authoring
Add/remove `influenced_by`/`related` (and the rare `parent` fallback) edges on a node, inline in the ego-view → `POST/DELETE /api/genres/{id}/edges`.

## Open questions

1. **`post_genres` key** — slug vs a synthesized post id (the `blog` collection has no DB row). Slug is simplest but renames break the link.
...
## Appendix A — the seed palette (~220, by parent)

Notation: `✓N` = already present in the catalog (artist-count); `→X` = `influenced_by`/`related` edge to tier-0 parent X (filed under the **primary** parent listed). Cuts applied (Bolero, Snap, Turreo, Smooth Soul, Country Folk, Sonata, Oratorio, Library Music, TV Soundtrack, J-Traditional, Tropical, Europop).

**K-Pop** (3) — K-Pop/idol pop ✓81 · K-Ballad ✓134 · 4th-Gen/Noise K-Pop →Electronic
...
**Rock** (33) — Korean Rock ✓53 · Alternative Rock · Indie Rock · Punk · Post-Punk · Hardcore Punk · Grunge · Shoegaze · Post-Rock · Math Rock · Progressive Rock · Psychedelic Rock · Garage Rock · Art Rock · Noise Rock · Emo · Pop Punk · Classic Rock · Hard Rock · Surf Rock · Krautrock · Folk Rock →Folk-Country · Dream Pop →Pop · Jangle Pop →Pop · Power Pop →Pop · Heavy Metal · Black Metal · Death Metal · Doom Metal · Thrash Metal · Metalcore · Deathcore · Djent

**Electronic** (27) — EDM ✓11 · IDM ✓7 · Big Room ✓7 · Electroclash ✓7 · EDM Trap ✓6 →Hip-Hop · House · Deep House · Tech House · Progressive House · Electro House · Future House · Techno · Trance · Drum & Bass · Dubstep · Ambient · Downtempo · Trip-Hop →Hip-Hop · Synthwave · Vaporwave · Electro · Future Bass · UK Garage · Breakbeat · Hardstyle · Glitch · Drumstep
...
**Soundtrack** (7, *format axis*) — Soundtrack ✓14 · Film Score →Classical · Video Game Music · Anime OST ✓17 · Film Soundtrack · Musical Theatre · K-Drama OST
