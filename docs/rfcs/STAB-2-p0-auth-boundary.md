...
| 2026-06-06 | **Steps 4 + 5 DONE + verified in prod** (owner gave explicit `terraform apply` go-ahead). Merged ws#265/#263 + music#42 + front#95; `terraform apply` clean plan (5 add / 2 change / 0 destroy, no unexpected drift). **Step 4:** `/candidates` `Bearer x` → **401** + no-auth → 401 (was 200, **P6-2 closed**); public `/unified` → 200 (preserved). **Step 5:** `$default` `DefaultRouteSettings` = burst 100 / rate 50 (live). **STAB-2 COMPLETE** (Steps 1-5; the `POST /api/categories` write hole = Step 3, tracked under STAB-5). | Steps 4, 5 |
