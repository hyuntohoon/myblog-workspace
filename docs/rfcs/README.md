...
- **FEAT-lyrics-annotations** (draft — session-capture 2026-07-25, **not scoped**) — two lyrics-domain threads frozen from a brainstorm: (1) "가사 이야기" per-line commentary from **Genius annotations** (API-legal; Genius never a lyrics-text source — scraping/ToS), coverage-probe gated; (2) lyrics-text **coverage expedite** for a specific new album (existing `lyrics_reassessment` is stalest-first over ~12.5k → weeks of latency; needs album-scoping). All 2026-07-25 probe data (Charli XCX MFF/BRAT) preserved in-RFC. → `FEAT-lyrics-annotations.md`.
