...
- **FEAT-playback-bucket-player** (**accepted** 2026-08-03 — next = Step 2 shared_db) — one system-owned **Playback Bucket** per playback-eligible user: auto-created, non-deletable, non-duplicable, position-movable; tracks and albums in (albums expand to tracks **in album order**), artists rejected; **the bucket's item order is the queue** (auto-remove on completion, remove-current advances). Playback is **not rebuilt** — it consumes the `play(intent)` ladder, device picker, Media Session and navigation persistence that FEAT-member-player Step 5 **shipped 2026-08-02**. Plays by re-issuing its own tail as `play({kind:'uris'})`, so it never writes to Spotify's queue and there is nothing to reverse-sync. Two forms over one session (Pocket mini-player / expanded player, OQ2 compares three shapes) + single-tab playback ownership with measured failure recovery. Schema reuses the V40 per-user partial-unique idiom; the genuinely new backend work is a **system-bucket delete guard that does not exist today** (`spotify_library`/`to_listen` are deletable right now). **Owner decisions 2026-08-03**: members get both rungs (open the `streaming` scope — owned by `FEAT-member-player`), no expansion confirmation at any size + Undo, and lyrics **plus the Genius-derived annotations** open to members (reverses `FEAT-lyrics-annotations` §6.9 O2 — owned by the lyrics stream, and that step must also fix `models.py`'s now-false "owner-only research data" comment and delete front #319's 403 UI). One question stays open by design: the expanded-player form, decided at Step 6 after a side-by-side comparison. → `FEAT-playback-bucket-player.md`.
- **ARCH-entity-interaction-v2** (**accepted** 2026-08-03 — next = Step 1 audit; parallel-safe with the playback RFC) — third RFC in the entity-interaction lineage (contract → unify → v2). One written definition per entity of **canonical id / URL / open / data / actions / drag payload**, one typed `DragPayload` replacing the board-internal `DndItem`, and a per-target × per-entity **direct-add / transform / reject** declaration. Trigger: exactly **two surfaces in the product are draggable**. Preserves internal-MOVE vs external-COPY structurally; keeps `boardDnd.ts`'s proven rules and adapts around them; settles click/drag/scroll and the mobile path by **prototype, not library choice**. Reopens three decisions with an explicit "what changed since" — the two album-detail hosts (recommended default: **no further change**), the album canonical URL, and `TrackRow`'s reserved `play`/`add` slots (**granted**). `component-map.md` is measured stale, so Step 1 re-audits rather than extends. → `ARCH-entity-interaction-v2.md`.
- **ARCH-overlay-modal-isolation** (draft, 2026-08-06) — 2026-08-06 감사에서 확인된 오버레이 스크롤-락 입양 격차(27개 `role="dialog"` 중 16개가 `useScrollLock`을 아예 안 부름, 같은 파일 안에서도 갈림)와 `AddToBucketMenu`의 `role="dialog" aria-modal="true"` 선언 대비 실제 미구현(포커스 이동/트랩/복원/ESC/스크롤락 전무, 중첩 시 ESC가 밑에 있는 앨범 상세 모달을 대신 닫힘)을 다룬다. `ARCH-entity-interaction-domain-audit` Step 4(ESC/포커스트랩 `useDismissable` 이관)와 겹치지 않게 스크롤 락 + `AddToBucketMenu` 단일 컴포넌트 전면 수정으로 스코프 한정. → `ARCH-overlay-modal-isolation.md`.
...
- **FEAT-lyrics-annotations** (done 2026-08-03, **archived** — breadcrumb) — two lyrics-domain threads. Thread 1 (Genius "가사 이야기") is built out: the viewer slice shipped 2026-07-28 (fetch → V49 store → Korean bodies → owner-only read → in-sheet render), R1/R2/R4 landed 07-29 with the first real e2e closing the gate 07-30, and R6 + R3 both shipped 08-03. **R5 (song-description tier) was dropped by owner decision 08-03**: `description_ko` is read by no code in any repo and the store lacks the fingerprint/status columns the annotation path uses, so the "S" was really a migration-led cross-repo rollout — and the cheap prompt-side variant was rejected because 3 of 6 sampled descriptions are interpretation with nothing marking them apart from the citable ones (§6.13). The six-album KR/EN anchor regression scheduled for 07-29 had never run; run at close, it showed Korean anchors at **14–33% placed against an English control at 74–78%** — below the RFC's own <40% tier line, knowingly unfixed at 32 songs / 86 annotations (§6.6). Thread 2 executes under `DATA-catalog-noise-and-lyrics-coverage`; §3's measurements stay its evidence base and §9 lists every superseded claim. → `../archive/done/rfcs/FEAT-lyrics-annotations.md`.
