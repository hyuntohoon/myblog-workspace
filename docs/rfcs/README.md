...
- **FEAT-multi-user-accounts** (in-progress — **Phase 0~3a shipped + prod-verified 2026-07-08**) — 멀티유저 플랫폼: Google+Kakao 가입, RYM-style 전공개 앨범 평론, per-user 버킷/들곡, Last.fm 기반 청취, model-agnostic `LLMEngine` AI. `FEAT-member-dashboard` Step 6에서 분리. **Phase 0**(Identity min: shared_db V36 `users` + infra Cognito self-signup + backend `require_owner` gate + `/api/me` lazy-provisioning + front 가입 진입) + **Phase 1**(RYM reviews: shared_db V38 + backend #108 + front #257) + **Phase 2**(per-user buckets: shared_db V40 + backend #109 + ws #582) + **Phase 3a**(Last.fm: shared_db V41 + backend #110 + worker #67 + ws #583 + front #260/#261) 전부 출하. **Phase 4**(LLMEngine) 설계만 머지(#584) — 구현은 소유자 확인 전 대기. **잔존 소유자 잔무**: `LASTFM_API_KEY` SSM 설정(그 전까지 worker poll + EventBridge no-op, by-design). Gates G1/G2 per RFC. → `FEAT-multi-user-accounts.md` (Phase 4 → `FEAT-multi-user-accounts-p4-llmengine.md`).
...
- **FEAT-public-bucket-multiuser** (done + archived 2026-07-10) — "공개 버킷뷰어 + 멀티유저" 범위. **Scope A(공개 읽기전용 버킷뷰) 전체 출시·prod-live 2026-06-15** (A1–A5); **Scope B(멀티유저 계정)는 `FEAT-multi-user-accounts`에 완전히 흡수·구현됨** (V36 `users` + per-user scoping + `/members/[handle]` + Google/Kakao signup, 전부 prod-live). B5(social graph — follows/lists)만 post-launch future-work로 `FEAT-multi-user-accounts`에 이월. 본문 → `docs/archive/done/rfcs/FEAT-public-bucket-multiuser.md` (Scope A 요약은 `docs/archive/done/2026-06.md`).
...
- **FEAT-pocket-buckit-workspace** (done — closed + archived via #456, swept #508; **file removed** — breadcrumb) — front-only follow-on to the shipped Pocket Buckit: turned the tray into a real **multi-bucket workspace** (movable mini-drawers + edit/arrange mode) + lightweight tray read-model (stops full bucket-tree refetch on every nav). A+B DONE + prod-live; Step C deferred. 본문 → `docs/archive/done/rfcs/FEAT-pocket-buckit-workspace.md`.
...
- **ARCH-frontend-component-map** (done — closed 2026-07-02, archived → `docs/archive/done/2026-07.md`) — docs-only frontend component structure map (no refactor): two code-pinned artifacts — an LLM developer reference (real file paths, ownership, state/interaction/cache dependencies) + a human-readable route/feature/flow map. Verified 2026-07-02: routes-by-island table, domain ownership, and the **track-click verdict — no shared track-row component exists** (≥6 fragmented paths; only play funnels through the shared `requestPlayback` SDK layer, never the row UI). Includes a Component-map impact template for future RFCs. The shared track-click component is documented, NOT created. **Step 1 done 2026-07-02** — artifacts published at `docs/frontend/component-map.md` + `docs/frontend/structure.md` (the canonical living copies; the RFC's reference section is now a pointer). → `ARCH-frontend-component-map.md`.
- **RESEARCH-playback-product-architecture** (done — closed 2026-07-02, archived → `docs/archive/done/2026-07.md`) — docs-only decision research (not implementation): whether the current Spotify SDK/API integration supports a Spotify-like-but-simpler playback experience, or whether a separate custom playback RFC is necessary. Scoped to this project's flows (not a Spotify inventory). Separates confirmed capabilities / gaps (D28 blocks continuous position, not one-shot; no `track_id` on the snapshot → lyrics endpoint accepts `spotify_track_id` from the live read, server-side resolve) / verification (resolved 2026-07-02: position read is **client-side** `GET /v1/me/player`, mirroring `requestPlayback`'s existing call — rule #9 not engaged; remaining = one runtime probe on a live Premium session) / retain-vs-defer / recommendation: **no custom playback RFC** either way. **Step 1 done 2026-07-02 — live probe PASSED** (`GET /v1/me/player` → 200 with `progress_ms` + `item.id` on a live Premium session; idle = 204): FEAT-lyrics-viewer Step 3 binds the one-shot position read. → `RESEARCH-playback-product-architecture.md`.
- **ARCH-lyrics-normalization-model** (done — closed 2026-07-02, archived → `docs/archive/done/2026-07.md`) — docs-only (no schema preselection): how raw `lyric_plain` + raw `lyric_synced` (LRC) on `track_lyrics` normalize into **one** project-specific lyrics structure for FEAT-lyrics-viewer. Verified 2026-07-02 (prod probes): mixed = 91% of matched / plain-only = 9% / synced-only = 0 observed (defensive branch), V33 CHECK can force a synced-only row to store `lyric_plain=''` with text in `lyric_synced`, no normalizer exists (greenfield). One pure normalizer → one segment shape across plain-only/synced-only/mixed; **synced-only parsed to segments, never stripped to raw text**; **mixed derives from synced alone** (plain ≡ timestamp-stripped synced in 98%; ~2% mismatch → diagnostics, synced wins). Raw stays source-of-truth; read-time derived view. Future estimated sync / line restructure out of scope but not precluded. **Step 1 done 2026-07-02** — the exact payload spec (`availability`/`source_kind`/`trackable`/`normalizer_version`/`segments[]{i,text,start_ms}`) + `normalize_lyrics`/`parse_lrc` pseudocode + acceptance gate pinned in the RFC's "Normalized payload spec" section. → `ARCH-lyrics-normalization-model.md`.
- **FEAT-lyrics-corpus** (done — all steps prod-verified, closed 2026-07-02, archived → `docs/archive/done/2026-07.md`) — a **private, single-owner, correctness-first lyrics corpus** on tracks already in our catalog. LRCLIB = only lyric-text source (plain + synced; bulk ~28.6 GiB dump for local historical backfill + API for increments); MusicBrainz = artist-identity/alias + recording/version **evidence only**; lyrics.ovh excluded. Lyrics stay **private** — never in any public API/page/search/shared response (`artists.aliases` + `album_research` privacy precedent). **Album name is never a hard match key**; remix/live/demo/edit/cover/remaster/rerecording **never silently merged** — weak/ambiguous parks in `review_required`/`ambiguous`/`not_found`/`no_lyrics` rather than mismatching (correctness ≫ coverage). The local dump batch evaluates **only tracks already in this DB**; a `matched` lyric is never silently replaced without stronger evidence; periodic reassessment can promote unresolved rows as coverage grows. Metrics beyond precision: coverage, review-required rate, unresolved/no-lyrics rate, version-conflict false-match count (=0 target), source-quality conflict rate. **Translation / slang-meme / interpretation / album-level analysis = later separate RFCs** (this one builds only the substrate). Backend/worker/shared_db + local batch; no frontend, no public API. From the 2026-07-01 lyrics-corpus feasibility research. → `FEAT-lyrics-corpus.md`.
- **FEAT-lyrics-best-of-promotion** (done — all steps prod-verified, closed 2026-07-04, archived → `docs/archive/done/2026-07.md`; outcome: 98.1% of the 2,757-row parked pool promoted, corpus matched share 37%→49.5%, audit 40/40, exact-title untouched) — successor to FEAT-lyrics-corpus. Auto-promote the **single best plausible candidate** out of the conservative matcher's `ambiguous` / `review_required` pool as a tagged `best-of-ambiguous` / `best-of-review` `matched` row — coverage growth with **no manual selection UI**, **no change to `exact-title` matching**, **no schema change**, and **no new public exposure** (track_lyrics stays owner-only). The conservative matcher + its evidence are preserved verbatim; promotion is a separate pure layer run inside the shared `run_eval_batch` (so the incremental + reassessment jobs + any future backfill all promote). **v1 promotes on tier 1 only** (exact base-title + version agreement, body-carrying candidates only); tiers 2–4 (duration-nearest → fuzzy → richest) ship gated, opened only on Step-1 probe evidence; choice + reason recorded in `evidence.promotion` (fully reconstructable, reversible); best-of bases slot below `exact-title` in the replacement ladder so reassessment can upgrade best-of→exact as LRCLIB grows. Targets the ~2,783 parked rows (ambiguous 2,749 + review 34) + all future ambiguous/review outcomes; `not_found` is out of scope (nothing to choose). Step 1 gates the ladder on a live risk-quantification probe + labelled-sample precision. → `FEAT-lyrics-best-of-promotion.md`.
- **ARCH-entity-interaction-contract** (accepted) — front (entity × action) interaction contract: shared `TrackRow` (React islands only — the vanilla review tracklist is explicitly excluded) carrying the **static lyrics entry** as first consumer, + `lib/entityLinks.ts` `artistHref`/`reviewHref` helpers replacing 16+ hand-rolled sites (fixes live trailing-slash drift) + clickable artist names on member surfaces + orphan search-bar deletion. Purpose = future-session token cost (one contract point per action), not code conciseness; album-detail & add-to-bucket dedup rejected as non-goals. 3 steps, front-only. → `ARCH-entity-interaction-contract.md`.
- **FEAT-today-buckit** (done — all 6 steps shipped/merged + prod-live 2026-07-09, owner round-trip verified, archived → `docs/archive/done/rfcs/FEAT-today-buckit.md`) — 두 개의 buckit 스타일 홈 섹션: **`TodayAlbumBuckit`(오늘, 이 앨범들)** = release_date 월/일 == 오늘인 앨범("N년 전"), DB-only 공개 읽기; **`TodaySongBuckit`(오늘의 곡)** = 게시일에만 노출되는 owner 수동 픽(인라인 홈 컨트롤로 올리기/바꾸기/삭제, `require_owner` 게이트) + overlay 히스토리 모달. 백엔드 `daily_picks` store(V39 prod-applied) + 공개 GET/owner PUT·DELETE; 인프라 apigateway 라우트(apply 완료). 곡 타일이 앨범 타일 위(오늘의 곡 → 오늘, 이 앨범들). 본문 → `docs/archive/done/rfcs/FEAT-today-buckit.md`.
- **FEAT-release-calendar** (draft) — 두 공개 서피스. **Track A**: 홈 "새 앨범" 카드(≤30d 신보, DB-only off `album_ingest`). **Track B**: 신보 캘린더(catalog watchlist의 예정/발매 앨범·EP·싱글 — per-artist **MusicBrainz** future-date 쿼리 + `album_ingest` day-0 확장, DB-only read). Frozen `FEAT-new-release-feed` + 병렬 draft 흡수. Needs owner accept before Active. → `FEAT-release-calendar.md`.
- **FEAT-personal-release-tracking** (draft — 2026-07-08) — `FEAT-release-calendar`(#548)가 예약한 personalization follow-on: **개인 발매 피드**(추적 아티스트 리스트 + Buckit 후보 임포트) · Upcoming/Recently Released × Albums&EPs/Singles · per-user `user_artist_tracks` join. #548의 `artist_release_events` 위에 layer. 구현 계획 아님 — H1~H6 가설 + owner accept gate. Needs owner accept before Active. → `FEAT-personal-release-tracking.md`.
- **FEAT-multi-user-accounts-p4-llmengine** (design proposed — architect eval complete, **awaiting owner confirmation before implementation**) — 멀티유저 Phase 4 `LLMEngine` 설계 결정. 5개 `claude -p` 콜사이트를 하나의 `LLMEngine.run(job) -> LLMResult` ABC로 통합(engine home = `myblog_shared_db/.../llm/`, pin bump 0). 에러는 raised exception 계층(Transient/Validation/Quota). 이번 패스는 owner-central scaffolding만; BYOK/`user_api_keys`는 Gate G2로 연기. 소유자가 "시작" 명시 전까지 코드 착수 없음. → `FEAT-multi-user-accounts-p4-llmengine.md`.
