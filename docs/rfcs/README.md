...
- **FEAT-lyrics-annotations** (draft — **not yet accepted**) — two lyrics-domain threads, fully re-measured 2026-07-25. Thread 2: the unresolved-lyrics pool **diverges** (+180/day; drain 2.25/day at a 1.54% hit rate; 96-day cycle) and recency is a 32× signal once classical is excluded. Thread 1: Genius token issued and live-tested — **credits are language-independent (7/8 KR and EN), prose is not (1/8 KR)**, and annotation anchoring measures **74.7%** via `tools/genius_anchor.py`. §9 lists every superseded claim. → `FEAT-lyrics-annotations.md`.
