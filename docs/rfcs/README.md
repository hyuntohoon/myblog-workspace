...
- **FEAT-spotify-library-sync** (draft — shipped 2026-06-08, **archive pending owner**) — `review_buckets.kind='spotify_library'` 버킷이 주인장의 Spotify saved-albums Library 를 양방향 미러 (reqs 4/5/6). shared_db V15 + worker reconcile/Library client + backend sync/state endpoints + front 버킷 UI; 활성화 게이트 3개 적용 (apigateway route, 토큰 재스코프, `SPOTIFY_LIBRARY_WRITES_ENABLED=true` #290), prod-verified. 본문 → `FEAT-spotify-library-sync.md`; plan.md residual 대로 주인장이 archive → `docs/archive/done/rfcs/` 후 이 항목 drop.
