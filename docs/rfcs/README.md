...
- **STAB-4** (accepted) — STAB-1 §Seq item 4 구현: **schema.sql 백필 + 문서 drift** (P2-8/MODEL-12, P2-1/2/3/4, P1-7). **문서 전용.** prod introspection: 6개 테이블 누락·`posts.rating` (3,1)→실제 (3,2)·`library_items`(V7) 는 prod 부재 ghost → 백필 소스는 migration/ORM 이 아니라 `pg_dump`. `genres` 부재 → V13 free.
...
- **STAB-6** (accepted) — STAB-1 §Seq item 6 구현: **repo hygiene + remote tfstate backend** (P3-1..3-8). `bundle.zip`(26M)·worker `.gitignore` 트랜스크립트·`allure-results`(18)·music `.DS_Store`/`.idea`·front `.vscode` untrack; tfstate → S3+DynamoDB lock (로컬 전용 state 에 Cognito secret 평문). repo별 브랜치; Step 5 는 사람이 `terraform init -migrate-state`.
