...
- **FEAT-listening-history-import** (draft) — import the owner's Spotify **Extended Streaming History** (GDPR export, lifetime per-stream JSON: `ts` + `ms_played` + `spotify_track_uri`) into `spotify_stream_history` (shared_db V27) → the only path to true lifetime **play counts + listening time + per-era/per-year retrospective** (no Spotify API exposes counts/minutes). v1 = core + retrospective; post-import primary "favorite" signal = lifetime-play, 좋아요 = secondary. Ingestion = local `$0` script (no web-upload UI). Cross-repo, 5 steps. Source → `docs/reviews/2026-06-22/analysis-bucket-statsfm-research.md`. 미착수.
