...
- **OPS-delivery-safety-gates** (accepted 2026-07-23) — CI merge gates + deploy smoke + async failure boundaries. Backend `pytest` (46 files/631 tests incl. fail-closed auth regression) runs in **no** pipeline (audit O-1); worker/front/workspace lack blocking PR gates; no deploy smoke/rollback; async EventBridge crons have no DLQ. 5 steps (Step 1 backend pytest PR gate = P0; 2 worker/front gates; 3 deploy smoke; 4 async DLQ+alarms [infra]; 5 rollback runbook). Needs owner accept. → `OPS-delivery-safety-gates.md`.
- **REFACTOR-frontend-member-surface** (accepted 2026-07-23) — test net + shared fetch client + boundary extraction for the member surface. `BucketBoard.tsx` 2895 LOC/33 useState (audit C-1), **zero** frontend tests (C-5), 42 raw fetches across 7 clients with no shared timeout/cancel (C-6), lifecycle rule `crMeta` trapped in the mega-component (C-2). Steps 1–3 (vitest harness → single client → extract rule to `@lib/buckets`) = P1; Step 4 BucketBoard decomposition = **necessity-gated P2** (no OOP; may never open). Needs owner accept. → `REFACTOR-frontend-member-surface.md`.
