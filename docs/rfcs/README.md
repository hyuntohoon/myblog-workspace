...
- **FEAT-lyrics-sync-precision** (**accepted 2026-08-01**) — kill the lyrics viewer's fixed `SYNC_LEAD_MS = 300` by removing the two error sources it was silently cancelling (250 ms tick grain +125, `.lyv-line` 0.35 s opacity ramp +175), leaving ~80 ms labelled as taste. Step 1 boundary-scheduled focus + fast-attack transition; Step 2 event-driven re-anchor (pause awareness, `MYBLOG_PLAYBACK_CHANGED`, tab return) closing the three cases where the clock silently diverges; Step 3 `AudioContext.outputLatency` measure-first go/no-go. **No polling (D28 upheld), no stored/learned offset, frontend only.** → `FEAT-lyrics-sync-precision.md`.
- **FEAT-lyrics-viewer-playback** (draft — owner promotion pending) — transport bar (⏮ ⏸ ⏭) + full-screen queue swap inside the live lyrics viewer. Commands already exist in `sendPlayerCommand`; only the queue read is new. Queue is a screen swap, not a sheet, because the viewer's vertical drag is already line-stepping. Row tap jumps only where Spotify's `context_uri`+`offset` allows it. **Two OQs block execution**: the `user-read-currently-playing` scope (possible re-consent) and telling context tracks from user-queued ones. → `FEAT-lyrics-viewer-playback.md`.
