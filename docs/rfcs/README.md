...
- **FEAT-member-dashboard** (accepted, in-progress Step 2) — 회원 대시보드 `/profile`: 5탭(개요·평론·평론 버킷·라이브러리·통계), vertical-slice 멀티스텝. Step 1 frontend shell shipped. Step 2 라이브러리 탭(to-listen 새 모델 + reviewed derived view) 진행 중. Step 3 listening history + Spotify 연동(하이브리드 1h cron + 수동 새로고침). Step 5 nested-bucket backend + `/reviews/queue` 은퇴. Cross-repo from Step 2.
- **FEAT-genre-taxonomy** (draft) — 장르 브랜치 저작 v1: 2단 단일부모 트리(상위→하위), 생성+부모지정만, prod `artists.genres` 시드. Cross-repo (shared_db V7 → 시드 → backend API → contract → front 저작 UI). 앨범↔장르 연결·필터·통합뷰·관리는 v2+.
- **FEAT-header-scroll-prefs** (draft) — 사이트 전역 헤더 스크롤 거동 사용자 토글(3 모드: hide-on-scroll-down / compact-on-scroll / threshold-hide). 디폴트 hide-on-scroll-down. Frontend-only single repo.

Recently archived (2026-06): FEAT-review-bucket-board (평론 버킷 보드, 5 steps end-to-end, prod 16/16), FEAT-design-system-measure (design token scale + content measure unify to 800px + break-out hero + daisyUI removal + writer.css split), FEAT-post-edit-delete-ui (published-post edit/delete UI + restore flow). (2026-05): FEAT-reviews-redesign (Pitchfork-style `/reviews` index + filter island + stars-only cards), FEAT-view-redesign (lowfreq article read page, 6 steps cross-repo), FEAT-writer-lowfreq-redesign (writer flow end-to-end, 6 steps cross-repo), BUG-11/12 (writer search filter), BUG-14 (MB hangul alias), BUG-15 (MB false-match cross-check), BUG-17 (alias_fill per-row UPDATE), BUG-18 (MBID uniqueness pre-check), BUG-19 (writer music search cross-expansion), FEAT-search-grid-fix, FEAT-writer-cleanup. Earlier: ARCH-6 (shared DB, ADR 0005), ARCH-11 (absorb publish, ADR 0006), ARCH-12 (contract-first OpenAPI, ADR 0007). All in `docs/archive/done/rfcs/`.
