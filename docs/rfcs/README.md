...
- **FEAT-blog-to-review-migration** (draft — 구현됨, front PR open, **미머지**) — `/blog/* → /review/[slug]` URL 마이그레이션. `FEAT-home-redesign` Step 6 에서 분리. front-only: read route `git mv` → `/review/[slug]`, 모든 옛 `/blog/*` 에 no-404 정적 redirect stub(200+client redirect; 진짜 301 은 CloudFront Function = 선택적 owner-applied Step 2), `/blog/category` → `/reviews` 흡수, 내부링크+rss+sitemap 일괄 교체. lint/astro-check/build 통과; owner 수동 redirect 확인 + 머지 대기. → `FEAT-blog-to-review-migration.md`.
