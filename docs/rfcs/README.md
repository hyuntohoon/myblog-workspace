...
- **FEAT-editor-buckit** (draft) — **editorial commissioning desk**: a *pre-writing* idea generator (opposite funnel end from FEAT-ai-editorial-critique — proposes *what to write* vs critiquing an existing draft). **Stage 0 = a `$0` `claude -p` script → dated Markdown idea report** (`docs/editorial/ideas/`), no table/backend/frontend/scheduler/API key (clones `scripts/research_poller.py`, read-only DB). Grounds on review **buckets** (saved-but-unwritten items + notes + research notes; published reviews = **exclusion only** — grounding on reviews is "risky" at n=0), references via **live per-run WebSearch** surfaced as 참고 links (topic *similarity* fine; only structural guard = catalog-anchoring). n=0 unblocker; "no good idea right now" / "go write the bucketed album" are valid outputs. Stage 1 (album_research-style table + poller + `/profile` tab + draft-factory handoff) gated on Stage 0 + ≥1 review. From the editorial-engine direction (memory `project-editor-buckit-status`).
- **FEAT-genre-artist-distribution** (draft) — `/profile` 통계 charts: genre + artist/listen 분포. `FEAT-member-dashboard` Step 4에서 분리. `FEAT-genre-system`(genre) + `spotify_play_events`(listen) 의존. 미착수.
