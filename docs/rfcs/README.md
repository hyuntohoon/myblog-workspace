...
- **FEAT-today-buckit** (in-progress — **Steps 1–3 shipped 2026-07-08**) — 두 개의 buckit 스타일 홈 섹션: **`TodayAlbumBuckit`(오늘, 이 앨범들)** = release_date 월/일 == 오늘인 앨범("N년 전"), DB-only; **`TodaySongBuckit`(오늘의 곡)** = 게시일에만 노출되는 owner 수동 픽 + history. ARCH-entity-interaction-unify에 click 의존. Steps 1–2(music `on-this-day` GET #52 + front 타일 #256) 출하·prod-live; Step 3(shared_db V39 `daily_picks`, #55) 머지(소유자 apply 대기). NEXT = Step 4 backend pick store → Step 5 infra → Step 6 song 타일. → `FEAT-today-buckit.md`.
