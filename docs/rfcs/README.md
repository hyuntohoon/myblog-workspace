...
- **FEAT-public-bucket-multiuser** (draft — 설계/태스크분해만, 코드 없음) — "공개 버킷뷰어 + 멀티유저" 범위 (2026-06-14 read-only 조사 RFC). **결정 2026-06-14: Scope A(공개 읽기전용 버킷뷰)를 Scope B(멀티유저)와 분리해 먼저 출시 (OQ1=yes); Scope A 에 비공개 버킷 인증 게이트 포함 (OQ2=포함).** Scope A 순서: A1(`is_public` 마이그레이션)→A2+A5(공개 read 엔드포인트 + 비공개 버킷 JWT 게이트)→A3(프로필 공개토글)→A4(읽기전용 `/collection` 뷰어). 보안 모델 + `GET /api/buckets` 무인증 선결함 플래그 포함. → `FEAT-public-bucket-multiuser.md`.
