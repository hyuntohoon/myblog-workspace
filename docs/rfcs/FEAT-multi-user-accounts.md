...
| 2026-07-14 | **profile-merge PR3 SHIPPED + prod-smoked (owner: "모두 병렬로 이어서 작업" — soak waived by explicit continue; 2 codex-built tracks in worktrees, serial merges #279 → #280).** **PR3-a** front **#279**: SelfDashboard gains ProfileApp's lyrics overlay state verbatim (live LyricsViewer + static LyricsSheet, `?lyrics=` debug entry incl. close-strips-param, onOpenLyrics → OverviewDash/StatsTab/AlbumDetail; the FEAT-lyrics-viewer intent comment ported since ProfileApp's copy dies in PR3-b). Public MemberProfile chunk **byte-identical 8,196 B** (lyrics only in the lazy SelfDashboard chunk 2,564→3,521 B). Prod smoke: member session `/members/?me&tab=overview&lyrics=<Pony track>` → param survives self-resolve, SYNCED 42-seg sheet renders, console 0 (lone entry = pre-existing member 403 on owner-only `/api/playback/spotify-token`, graceful). **PR3-b** front **#280**: `/profile` → noindex redirect preserving `?tab=` (encodeURIComponent, no open-redirect); ProfileApp deleted (446 LOC); header avatar + selfPageHref() + PocketTray converge on `/members/?me` all roles; `/profile-reviews.json` (owner feed source) + `profile.guard.ts` (settings.astro) retained. Prod smoke: `/profile/?tab=bucket` → member My Buckit board, 0 `/profile` anchors, console 0. Reviewer 3-way merge check: 0 conflicts. **Pocket ▶ gate residual CLOSED**: prod anon (no ▶/write/오늘의곡 controls) + smoke-member with a REAL bucket item (Pony) — drawer renders the album row with **no ▶**; test bucket deleted (204). NB the first member-tray render came up empty because only `access_token` was seeded — `bucketStore.userScope()` keys off `id_token`; seeding both (real-login parity) renders correctly. Verification artifact, not a prod bug. `/buckets` flag resolved by design (every role → `/members/?me&tab=bucket`, now the only self-dashboard). Remaining: owner live-login returnTo observation; cosmetic member 403 probe on the owner playback-token route; OQ4-era backlog unchanged | front #279/#280; ws docs this PR |
| 2026-07-14 | **Minor-backlog + mutation-cap batch SHIPPED + prod-smoked** — front #281 (owner-gate the playback-token mint → members fire zero 403 probes; `isPlaceholderIdentity()` renders `user-<sub8>` as mono `@handle` on AlbumRatingBlock/MembersHub/MemberProfile) + backend #119 (`REVIEW_DAILY_CAP` pattern → `POST /api/buckets` 30/24h + `POST /api/buckets/{id}/items` 500 rows/24h; integrations connect uncapped = upsert-one-row-per-provider has no attempt trail; openapi zero-diff, 499 pytest). Closes the 07-14 residuals: member-403-probe (cosmetic), rate-limits-beyond-reviews, `user-<sub8>` display. Canonical link (runtime `?u=` vs static `[handle]`) deferred — 0 static member pages prebuilt (empty `/api/members`) ⇒ 0 dup URLs today (necessity-gate). Remaining residual = owner live-login returnTo observation only | front #281 + backend #119 |
