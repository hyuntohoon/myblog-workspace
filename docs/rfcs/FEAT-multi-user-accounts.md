...
6. **Member-page reachability model (blocks the rescoped profile-merge PR3)** — found
   2026-07-14 audit: `/members/[handle]` is `getStaticPaths`-prebuilt from `GET /api/members`,
...
7. **Last.fm listening attribution** — username-only connect means any member can claim any
...
8. **Bucket publicness model vs decision 3** — decision 3 says "everything public, no
...
9. **Owner editorial vs community ratings (two rating systems)** — `/canon` (명반 전당, ★4.0+
